\begin{tikzpicture}[
  ...
  bracket/.style={inner sep=2pt, fit=#1}
]
...
\tikzset{
  computer/.style n args={1}{%
    ...
    path picture={
      ...
      \draw[fill=\pgfkeysvalueof{/tikz/computer fill}] (1,0.125) rectangle (1.25,.25);
      % Draw the base rectangle.
      \draw[fill=\pgfkeysvalueof{/tikz/computer fill}] (0.625,0) rectangle (1.625,0.125);
...
% ------------------------------------------------------------------------
\node[bigbox, label={[titlebox, xshift=0.cm, yshift=.75cm]\textcolor{black}{\textsc{Moment Unfolding}}}](RANBox)
      at (4.875, 1) [minimum width=19.2cm, minimum height=9.2cm] {};

\node[bigbox, label={[titlebox, xshift=-1.cm, yshift=-1cm]
       \textcolor{green!40!black}{Particle Level}}] (particleBox)
      at (1,1) [minimum width=9.2cm, minimum height=7.2cm] {};

\node[bigbox, label={[titlebox, xshift=1cm, yshift=-1cm]
       \textcolor{blue!60!black}{Detector Level}}] (detectorBox)
      at (9.5,1) [minimum width=7.2cm, minimum height=7.2cm] {};
...
\node[align=center, opacity=0.75] (truth) [below=1cm of gen, xshift=4.5cm, inner sep=0pt, 
    label={center:{\shortstack{\textbf{Truth}}}}]{
    \includegraphics[width=0.145\linewidth]{figures/chapter-01/Collision.pdf}
};

\node[bracket=(truth)] (brackets) {};
\node[left=0pt of brackets.west, anchor=east] {\(\scalebox{1.5}{$\mathbb{E}$}\, \vast[\)};
\node[right=0pt of brackets.east, anchor=west] {\(\vast]\)};

% Re-weighted Generation (bottom)
\node[box, fill=green!10]
  (regen) [left=2.5cm of truth]
  {\textbf{Re-weighted}\\\textbf{Generation}};

\node[bracket=(regen)] (brackets) {};
\node[left=0pt of brackets.west, anchor=east] {\scalebox{1.5}{\(\E\)}\( \vast[\)};
\node[right=0pt of brackets.east, anchor=west] {\(\vast]\)};

% Arrow from Truth to Generation (optional, if needed conceptually)
\draw[draw=none] (regen) to node[pos=0.4]{\scalebox{1.5}{\(\approx\)}}(truth);
...
\node[align=center, opacity=0.75] (data) [below=0.9cm of sim, xshift=-1.5cm, inner sep=0pt, 
    ...
    \includegraphics[width=0.145\linewidth]{figures/chapter-01/Collider.pdf}
};

% Re-weighted Simulation (bottom)
\node[box, fill=blue!10]
  (resim) [right=.5cm of data]
  {\textbf{Reweighted}\\\textbf{Simulation}};
\draw[draw=none] (data) to node[pos=0.5]{\scalebox{1.5}{\(\approx\)}}(resim);
...
\draw[arrowstyle,dashed,
  decoration={
    markings,
    mark=at position 0.45 with {\node[above, rotate=17, font=\normalsize]{{Induced weights \(g(z,\vb*\beta)\)}};}
  },
  postaction={decorate}
]
  plot [smooth, tension=0.0] coordinates {(regen.north east) (sim) ($(resim.north)+(0.2, 0)$)};
...
% ------------------------------------------------------------------------
\draw[arrowstyle, dashed, red!70!black]
  (dLabel) to[out=200, in=20] 
  node[pos=0.7, above, sloped, text=red!70!black, font=\normalsize]
    {Backprop Update}
  ($(regen.east)+(-0.5,1.5)$);
