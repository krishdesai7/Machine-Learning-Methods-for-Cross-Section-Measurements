\chapter{Reweighting Adversarial Networks.}
\label{chap:ran}
\chapterminitoc
\section{The need for full spectral measurements.}
    ...
    Binning an observable into histograms can be a more detailed approach than quoting a small number of moments, but it too imposes significant limitations.
    %
    These have been discussed at length in previous chapters, but this section recapitulates them as a reminder, and connects them to the limitations of moment focussed methods as well.
    ...
    %
    Hence, to fully exploit the data and enable the broadest possible comparisons to theory, it is imperative to go beyond a few moments or fixed bin histograms and aim to unfold the continuous differential cross section itself.
    \begin{description}
        \item [Information loss.]
            ...
            %
            Fine binning could, in principle, mitigate this, but the detector resolution and finite statistics limit one's ability to use bins that are too fine.
            %
        ...
        \item[Binning artifacts and biases.]
            ...
            Furthermore, when binning multivariate data\footnote{Or examining an observable's distribution in bins of another quantity.}, the necessary discretisation in each dimension can introduce artificial discontinuities~\cite{GomezAmbrosio:2022mpm, Fiorendi:2014klw, KLOE-2:2025ggc, Ressegotti:2024szk, LHCb:2025eyf, Grosso:2024nho,}.
            %
            These artifacts hinder comparisons of unfolded results with theoretical predictions or with results from other experiments that used different binning schema~\cite{Brehmer:2018hga}.
            ...
            %
            If the true distribution varies non-linearly inside the bin, the extracted moment is biased by the bin size and by the shape assumption, such that even the sign of the error cannot be known \textit{a priori}~\cite{desai2024moment}.
            %
            An unbinned measurement eliminates this intermediate step and the bias that can arise from it.
            
        \item[Limited dimensionality.]
...
\input{tables/6-1.tex}
    ...
    \cref{tab:meas_compare} summarises these differences between unbinned moment based unfolding, binned density unfolding, and unbinned full spectrum unfolding methods.
    %
    ...
    A few examples are:
    \begin{description}
        \item [Jet substructure observables:]
            %
            ...
            If on\-ly the av\-er\-age jet mass or a few quant\-iles were re\-por\-ted, one wou\-ld lose the struc\-tur\-al in\-for\-ma\-tion of how often jets are hea\-vy ver\-sus light, or how sub\-struc\-ture sculpts the dis\-tri\-bu\-tion.
            %
            Si\-mi\-lar\-ly, dis\-tri\-bu\-tions of \(\tau_{21}\)\footnote{A ratio used to tag two prong substructure.} contain information about the fraction of jets with two subjects versus one, which is crucial for signal (e.g. boosted $W$) vs background (QCD jet) discrimination.
            %
        ...
        \item[Charge and flavour sensitive observables:]
            %
            ...
            %
            It is a particularly challenging observable to unfold because it is strongly sensitive to soft QCD and hadronisation effects~\cite{Lyons:2011cli, Grosse-Oetringhaus:2009eis, ALICE:2022xip, Segura:2024srj, STAR:2022etb, Carli:2015qta, Skands:2014pea, Christiansen:2015yqa}.
            %
        ...
        \item[Multidifferential and high dimensional measurements:]
            ...
            %
            With traditional methods, one would perform a two dimensional unfolding, choosing a projection \(Q_j\) of \(\vb*Q\) if needed (bins in $O$ vs bins in $Q_j$) in order to then extract moments or other features as a function of $Q$~\cite{CMS:2011xqa, ATLAS:2010jvh, ATLAS:2020ccu, CMS:2014eaj, ATLAS:2022zbu, DAgostini:265717}.
            %
            ...
            %
            This is especially relevant in the era of high luminosity experiments, where huge datasets invite more differential measurements.
            %
    ...
    \subsection{Challenges in unfolding full distributions.}
        ...
        %
        Infinitely many particle level spectra are, in principle, consistent, within uncertainties, with a given set of detector level data, especially if the data are treated in fine detail.
        %
        The problem of small fluctuations or statistical noise in the detector level histogram causing huge oscillations in the unfolded solution is exacerbated when the response ``matrix'' is essentially continuous, since here one is trying to reconstruct a function rather than a finite vector, which has infinitely many degrees of freedom.
        ...
        %
        Traditional regularised unfolding (e.g. Bayesian iterative methods or Tikhonov regularisation) starts from a initial guess for the Truth distribution and updates it in light of the data.
        %
        If the ansatz is significantly wrong in a region where data have low sensitivity, the unfolded result may inherit that wrong shape (bias) because there isn't enough information in the data to correct it.
        %
        For binned methods with strong regularisation, the unfolded spectrum can end up looking very much like the ansatz except in regions where the data clearly indicate otherwise.
        %
        When unfolding a full spectrum, managing bias can be even trickier than when unfolding moments.
        ...
        %
        This need for sufficient overlap in the support of the initial assumption and Truth distribution is a fundamental mathematical limitation: full spectral unfolding demands that the Generation model be flexible and broad enough to encompass the Truth, at least roughly, or else certain features will be missed entirely.
        %
        ...
        %
        For example, unfolding 100 bins will yield each bin count with larger uncertainty than if one had combined them into 1 bin, since the the bin count of 1 bin is simply the total count.
        %
        ...
        %
        Even if one had infinite amounts of data and so unpopulated bins were not a concern, these traditional algorithms like IBU that are relatively fast for tens of bins become very slow for hundreds of bins, and practically unusable for thousands of bins or continuous data, as each iteration must refine a fine grained distribution.
        %
        ...
        %
        These computations require substantial computing resources (CPU and GPU), careful hyperparameter tuning, and can suffer from convergence issues.
        %
        Ensuring that an unbinned unfolding converges to a stable solution without excessive computation is a highly non-trivial challenge.
        %
        Thus practical feasibility of unfolding full spectra depends on advances in algorithms and computing, precisely the advances that modern machine learning based methods aim to provide.
    ...
    \begin{note*}
        This section focusses specifically on the regularisation methods that recent ML based unbinned unfolding approaches have successfully employed. For a broader conceptual overview of unbinned unfolding see \cref{sec:theoretical-foundations.unbinned-methods} and for a survey of ML based unbinned unfolding methods, see \cref{sec:ml-for-unfolding.case-studies}.
    \end{note*}
        ...
        %
        Broadly, these approaches fall into two categories, those based on \textit{reweighting} an existing MC sample to better agree with nature, and those based on \textit{generating} new events\footnote{Typically with generative models.} to reproduce the Truth distribution.
        %
        ...
        %
        This procedure is iterated at both detector and particle level in alternation.
        %
        ...
        A noted limitation of \textsc{OmniFold} is its computational cost: each iteration requires training two classifiers to convergence, and in realistic cases one might need $\numrange{5}{30}$~\cite{STAR:2022etb} iterations, amounting to training a large number of networks.
        %
        The iterative nature can also complicate uncertainty evaluation and hyperparameter tuning.\footnote{E.g., one must subjectively choose when to stop iterating to avoid instability.}
        %
        Still, \textsc{OmniFold} set the stage for practical unbinned unfolding and proved the feasibility of full spectral measurements on real experimental data.
        ...
        %
        Pure generative unfolding would require vast amounts of data to constrain the high dimensional space and can suffer from mode dropping.\footnote{The generator failing to produce some less common features of the distribution if not properly incentivised.}
        %
        ...
        Between pure reweighting (which uses an existing simulation as a starting point) and pure generation (which starts from random noise) lies an attractive compromise: using machine learning to \emph{reweight} an initial MC in a single pass, by directly comparing its predictions to data.
        %
        The \textbf{Reweighting Adversarial Network (RAN)} method is built on this approach.
        %
        ...
        The challenge is that $p(z)$ cannot be directly---one only has access to its smeared version at detector level.
        %
        Therefore, a RAN sets up a two level training objective to optimise $g(z)$ based on how well the weighted events, after going through the detector simulation, agree with the detector level data.
        %
        One might think that such an approach would be computationally unfeasible due to having to repeatedly rerun expensive detector simulation on reweighted Generation, and this would indeed be the case if the generator were modifying the features themselves.
        %
        The intrinsic pairing of (Gen., Sim.) events, combined with the fact that the generator only modifies the importance weights of events rather than the events themselves means that the detector emulation only needs to be run once to create the original Simulation dataset, after which, Simulation can simply inherit its weights from the matching Generation event.

        \cref{tab:unfold_methods} provides a high level comparison of these methods.
        %
        ...
        %
        Notably, most current ML based methods still require that the starting Simulation has support in the regions of interest of the Data.
        %
    ...
    \subsection{Addressing challenges.}
        ...
        The Reweighting Adversarial Network (RAN) approach described in this chapter is designed to address the aforementioned challenges head on, enabling full spectral unfolding with improved stability and efficiency.
        %
        This section outlines how the methodology of RANs addresses the needs and difficulties detailed above.\footnote{A complete description of the method follows in later sections, so the focus here is on conceptual understanding rather than implementation details.}
        
        A RAN explicitly targets the entire distribution rather than a fixed set of moments.
        %
        ...
        %
        By using a flexible neural network to parametrise the weighting function, a RAN does not impose a rigid functional form limited to a few moment constraints.
        %
        This means it has the capacity to adjust the simulation such that not only the first few moments, but rather \emph{all} features of the distribution\footnote{In principle, every differential element.} are brought into agreement with the observed data.
        
        Framing the problem in this fashion ensures that when a RAN converges, the theoretical guarantees of \cref{subsec:theoretical-properties.analytic-closure} result is a faithful unfolded spectrum, from which moments or any other summary statistic can be derived.
        %
        Importantly, because it handles the full spectrum at once, RANs avoid the bias of selecting certain observables upfront---it lets the data inform the entire shape. 
        %
        Hence the RAN method is aligned with the basic motivation of full spectral measurements: nothing gets thrown away or averaged out prematurely.

        By construction, the RAN algorithm is an unbinned method.
        %
        ...
        %
        If it finds any differences in the distributions, no matter how localised, the reweighting network is encouraged to adjust weights (at particle level) to eliminate that discrepancy.
        %
        ...
        As a result, RANs do not suffer from the bin alignment issues or interpolation biases that plague binned unfolding.
        %
        Different experiments using RANs on the same observable should, in principle, get comparable results without worrying that one used binning scheme A and the other binning scheme B, since neither would use bins at all.
        %
        The output of a RAN can be presented as a smooth distribution or as a weighted event sample at particle level, which can then binned or analysed downstream as needed.
        %
        This flexibility maximises the utility of the measurement.

        One of the core design goals of the RAN method is to eliminate the need for multiple iterative reweighting cycles (as in \textsc{OmniFold}).
        %
        ...
        %
        In practice, this means a RAN involves training one pair of neural networks with feedback from each other in one integrated run.
        ...
        %
        Furthermore, by using an optimal transport based loss (inspired by the Wasserstein GAN) and techniques like spectral normalisation in the discriminator, a RAN achieves stable training dynamics.
        ...
        %
        These choices were crucial technical innovations needed to extend \textsc{Moment Unfolding} to a full spectrum unfolding method.
        %
        The end result is that a RAN converges reliably to a solution where the weighted generation agrees with truth, all in a single training loop.
        %
        ...
        Like any reweighting based approach, a RAN does rely on the distribution of MCMC samples and will fail if the Truth distribution lies entirely the support of the Generation.
        %
        However, a RAN upon convergence, provably does not rely on the exact Generation shape within the supported region i.e. it is unbiased, and converges to the MLE.
        %
        The use of an adversarial loss means that a RAN is effectively solving a constrained optimisation to find the weight function that makes the simulation statistically indistinguishable from data by reweighting only at particle level.
        
        The RAN method's connection to the Boltzmann entropy inspired \textsc{Moment Unfolding} provides a theoretical understanding of how it converges the ``maximum entropy'' solution consistent with the constraints (the data).
        %
        This is desirable because the maximum entropy solution is the least biased one given the information at available.
        %
        In essence, a RAN inherits \textsc{Moment Unfolding}'s theoretical guarantees and compensates for the larger freedom of full spectra by adding proper regularisation in training, thereby taming the ill posedness.

        An important practical aspect is that a RAN yields physically reasonable unfolded distributions, such as non negative weights and normalised total cross sections.
        %
        Parametrising the weight function as $g({z}) = \frac1P\exp(-f({z}))$ (one convenient choice) ensures $g({z})\ge 0$ for all events, so the unfolded cross section is positive definite.
        %
        ...
        Moreover, because a RAN operates at the level of reweighting events, the resulting weighted event sample can be validated easily.
        %
        One can always forward fold the weighted sample through the detector emulation to check that it indeed reproduces all features of the observed data.
        %
        ...
        \subsubsection{Subtracting backgrounds with RANs.}
        ...
        While this extension has not been implemented or tested in the current work, allowing negative weights could potentially enable joint background subtraction and unfolding.
            ...
            %
            Development of these capabilities would require careful validation and is left for future work.

        In summary, a RAN is designed to meet the need for unfolding multivariate probability densities by addressing the key challenges that have historically prevented such measurements.
        %
...
\sectionminitoc
    ...
    %
    This section now builds a rigorous framework linking distribution moments to the full probability density, and details how leveraging moment constraints can facilitate unbinned unfolding of densities.
    ...
    %
    This section first develops the theoretical foundation connecting moments and probability distributions,\footnote{Including moment generating functions and maximum entropy arguments.} then surveys classical and modern unfolding methods that rely on moment constraints.\footnote{Such as regularisation and entropy based techniques.}
    %
    ...
    %
    Finally, the \textsc{Moment Unfolding} method introduced in \cref{chap:moment-unfolding} will naturally be extended toward unfolding the entire distribution (all moments) via a GAN--like approach inspired by the Boltzmann distribution, setting the stage for unfolding a full, unbinned, differential cross section measurement.
    ...
    \subsection{Moments and the full probability distribution.}
        ...
        In principle, an infinite sequence of moments completely characterises a distribution.
        %
...
\noproofref
\label{thm:moment-determinacy-mgf}
...
\end{theorem}
        ...
        %
        In fact, knowledge of $M_Z(t)$ (or the characteristic function) allows one to reconstruct the probability density $p(z)$ fully.
        %
        In short, an infinite set of moment constraints is mathematically equivalent to knowing the complete distribution.

        In practice, however, one can only estimate a finite number of moments from data with finite precision.
        %
        ...
        %
        For example, binned event counts are themselves integrals of the true spectrum over bin ranges and therefore a kind of `moment'.
        %
        ...
        \subsubsection{The maximum entropy principle and exponential families.}
            ...
            %
            Indeed, if $f_a(z) = z^a$ (the monomials), the above $p^*(z)$ reduces to the Boltzmann ansatz with coefficients $\beta_a$ related to the distribution's moments.
            %
            This exponential family is central to the moment based spectrum unfolding method.
            %
            It provides a flexible yet principled parametrisation of the true distribution in terms of a set of parameters ${\lambda_a}$ or equivalently a set of moments.
            %
            ...
            %
            Hence the full differential cross section can be unfolded to arbitrary precision in the limit of sufficiently many moment constraints.
    ...
    \subsection{Unfolding with moment constraints: classical and modern approaches.}
        ...
        \subsubsection{Linear regularised unfolding (Tikhonov and SVD).}
            ...
            %
            Although this has been discussed in \cref{chap:theoretical-foundations}, one is now equipped to reformulate the problem in the language of moments.
            %
            ...
            %
            By keeping only the dominant modes, essentially the lowest frequency or moments with largest norm, SVD unfolding ensures stability at the cost of not fully utilising high frequency information.
            
            Both Tikhonov regularisation and SVD thereby regularise the moment space, implicitly limiting the effective number of moments that contribute to the unfolded solution.
        ...
        \subsubsection{Iterative Bayesian unfolding.}
            ...
            %
            It starts with an initial guess for the true distribution, Generation, which corresponds to some initial moment estimates and iteratively updates the expectations to better agree with the observed data.
            %
            ...
            This procedure can be interpreted as ensuring that the predicted detector level counts, moments in a top-hat basis for each bin, match the observed counts, one step at a time.
            %
            ...
            The regularisation can, then, be interpreted as limiting the effective degrees of freedom (moments) that are fitted, similar to Tikhonov or SVD albeit via a different mechanism.
        ...
        \subsubsection{Entropy based methods.}
            ...
            %
            Entropy regularisation thus penalises any extraneous moment fluctuations that are not required by the data, biasing toward a Gaussian distribution in the absence of strong evidence for shape.
            ...
            %
            By treating the deviation from an ansatz in terms of Kullback--Leibler divergence (relative entropy), one can incorporate prior knowledge while still preferring the least structured solution beyond that prior.
            %
        ...
        \subsubsection{Unbinned methods through the lens of moments.}
            ...
            %
            While \textsc{OmniFold}, for example, does not explicitly constrain low order moments or use an analytic parametrisation, it implicitly matches all features of the distribution that the classifier can discern---effectively attempting to equate the full set of moments by the end of the procedure.
            %
            In each iteration, the classifier focuses on the current differences between data and weighted simulation, which are often in some \emph{mode} of the distribution; iterating allows successively finer differences, higher order moments or localised shape features to be corrected.
            %
            ...
            Other approaches employ adversarial neural networks or optimal transport metrics to directly learn the unfolded distribution in one go, rather than iteratively, using a generator model for the true distribution and a discriminator to enforce that the generator's folded output looks like the data.
            %
            ...
            %
            These methods still must address ill posedness\footnote{E.g. through network architecture choices, training tricks, or implicit regularisation like early stopping}, but they offer a way to directly target the entire set of distributional degrees of freedom rather than pre-selecting a fixed set of basis functions.
    \subsection{Benefits and challenges of \textsc{Moment Unfolding}.}
        ...
        %
        The variance of estimators is lower since one has effectively integrated over many events to get each moment.
        %
        ...
        %
        In multi-differential measurements with many kinematic variables, fully binning the data becomes impractical due to sparsity (curse of dimensionality), but one might still meaningfully measure lower dimensional moments, such as the average of some observable as a function of another variable, avoiding the need to populate multi dimensional histograms.
        %
        ...
        In this way \textsc{Moment Unfolding} shifts the ill posedness to the choice of moments.
        %
        ...
        %
        This is connected to the concept of model dependence---choosing an insufficient set of moments imposes a choice of model on the unfolding result, thereby biasing it.
        ...
        %
        This is analogous to the bias--variance trade-off in classic unfolding: using fewer moments yields a biased but low variance estimate; using more moments approaches an unbiased full result but with higher variance and higher risk of over fitting data fluctuations.
        
        Another limitation of any reweighting based unfolding, moment based or otherwise, is the requirement of support overlap.
        %
        The method can only correct distributions within the support of the MC distribution.
        %
        ...
        %
        Thus, \textsc{Moment Unfolding}, like other reweighting methods, assumes that the Generation's phase space is broad enough to contain the Truth or that any deficiencies are corrected by other procedures.
        %
        It is worth noting that as one increases the number of moments or attempts to unfold the full distribution, the demand on simulation support becomes stricter: essentially all features of the Truth distribution must be present in the Generation to be recovered by reweighting.
        
    \subsection{Extending \textsc{Moment Unfolding}.}
        ...
        Another perspective is to see $g(z)$ as the Radon--Nikodym derivative between the true distribution $p(z)$ and the generation $q(z)$, restricted to the family of functions parametrised by $\beta_a$~\cite{NikodymSurRadon, radon_theorie_1913}.
        %
        If the true distribution $p(z)$ lies within this family for the chosen basis set, then there exists some $\boldsymbol{\beta}_*$ such that $p(z) = g(z; \vb*{\beta}_*),q(z)$.
        %
        Even if $p(z)$ is outside this family, we expect that with a sufficiently rich set of basis functions, one can approximate $p(z)$ in the moment sense.
        %
        ...
        \[
            \E_{\text{unfolded}}\qty[Z^k] = \int z^k\,g(z;\,\vb*\beta)\,q(z)\,\dd z ,
        \]
        or in practice, the ratio of weighted sums over all generation events,
        \[
            \E[Z^k] = \sum_{z\in \text{Gen.}} z^k\;g(z;\, \vb*\beta)\;.
        \]
        ...
        %
        \(g\) incorporates the normalisation factor $P(\boldsymbol{\beta})$ in the \(g\).
        %
        This ensures that $g(z;\,\vb*\beta)$ does not arbitrarily change the total number of events.
        %
        ...
        %
        It guarantees that the reweighted distribution properly normalises to the total cross section or total event count expected.
        
        There is no explicit formula linking $\beta_a$ to a simple goodness of fit measure at detector level, because the detector response $r(x\mid z)$ is typically a complex stochastic mapping.
        %
        Instead, the machine learning training implicitly evaluates and drives this optimisation.
        
        The \textsc{Moment Unfolding} method was demonstrated to yield accurate and statistically stable moment estimates in realistic scenarios.
        %
        By avoiding any intermediate binning, it circumvents bin size biases and fully utilises the fine grained shape information in the data.
        %
        ...
        %
        In essence, it reduces the unfolding task to a parameter fitting problem under the hood, albeit a high dimensional, simulation informed and adversarially solved one. This focus provided a built in regularisation through the number of moments unfolded.
        %
        The results in \cref{sec:moment-unfolding.case-studies} showed that even with $n$ as small as \(2\) one can capture key shape characteristics of distributions.
        ...
        The normalisation factor $P(\boldsymbol{\beta})$ becomes increasingly complicated to compute or differentiate when $\boldsymbol{\beta}$ is high dimensional, and the adversarial training becomes increasingly unstable due to the number degrees of freedom to correctly learn.
        %
        ...
        %
        RANs can be seen as \textsc{Moment Unfolding} taken to the continuum limit, where instead of a few predetermined moments, all aspects of the distribution are learned.
        %
        Practically, a RAN forgoes the explicit regularisation of $g(z, \vb*\beta)$ with a fixed set of $\beta_a$ multiplying known basis functions.
        %
        ...
        The adversarial setup remains, but now the generator’s parameters are not directly interpretable as specific moments;\footnote{Although the moment interpretation can be recovered by computing the Taylor coefficients of the log generator.}
        %
        they are a more granular description of the weight function.
        %
        The discriminator in a RAN is tasked with comparing the fully continuous shapes of $\qt(x)$ and $p(x)$, in theory, enforcing an infinite number of constraints, since matching two distributions means matching an infinite set of moments or test statistics.
        %
        In this way, a RAN builds on the moment based approach by removing the artificial limit on the number of moments: the method `learns to unfold all the moments of a distribution'.

        One important innovation in the development of RANs was the use of the Wasserstein GAN framework~\cite{stephanovitch_optimal_2023}, which provides a stable way to train the adversarial system even when the generator distribution and true distribution initially differ significantly.
        ...
        %
        In a naive GAN, if $g(z)$ significantly changes normalisation, the discriminator could easily detect a total rate difference, and the generator would then simply rescale its prediction to match the event counts, potentially neglecting shape differences.
        %
        By using an optimal transport loss and carefully constraining $g(z)$ to keep the total weight near unity, a RAN avoids trivial solutions and focuses on shape corrections.
        %
        In other words, if a RAN does not incorporate the normalisation into its architectural design, then the architecture can lead to misestimations of the normalisation, and the RAN will be have to learn the correct normalisation, at the expense of learning other features. 
        
        Although this was also true of \textsc{Moment Unfolding}, there is a quantitive difference.
        %
        When $n$ was small, a few $\beta$ primarily modify the shape of a distribution within a highly restrictive scheme.
        %
        Misestimations of the partition function become increasingly relevant when $g(z)$ becomes a free form function.

        By allowing a more expressive reweighting function, a RAN is able to unfold the entire distribution in an unbinned, non-iterative manner.
        %
        ...
        %
        A RAN achieves a similar end point in one training loop by leveraging an adversarial paradigm.
        %
        ...
        %
        The implementation of a RAN thus requires additional care in training\footnote{E.g., gradient penalty terms for WGAN, spectral normalisation, etc.} to ensure convergence to a physically sensible solution that generalises and does not chase statistical fluctuations.
        %   
        The success of RANs in toy examples and simulated jet data applications demonstrates that it is indeed possible to go from moments to complete spectra.

In this way, the progression from \textsc{Moment Unfolding} to full distribution unfolding can be seen as a continuum.
%
At one end is a highly stable method focusing on a few moments; at the other end, a method that aims to extract the entire distribution, which requires stronger techniques to regularise.
%
\textsc{Moment Unfolding} paves the way to the more ambitious goal of unbinned differential cross section measurement via its extension to RANs.
%
...
\sectionminitoc
    ...
    \subsection{Historical approaches to regularisation.}
        ...
        %
        Often, the regularisation in ML approaches is \emph{implicit}, coming from network architectures and training protocols.
        
        For example, deep neural networks have a finite capacity and tend to learn simpler patterns first, a form of Occam’s razor that can act as an ansatz.
        %
        ...
        %
        By limiting the number of iterations and monitoring when the updates become consistent with statistical uncertainty, \textsc{OmniFold} effectively regularises the solution.
        %
        If iterated to completion, an algorithm like \textsc{OmniFold} would overfit the data.
        %
        This is analogous to IBU; halting earlier mitigates this risk.
        ...
        %
        Recent studies have integrated exact symmetry constraints into normalising flow or invertible network architectures used for unfolding, so that the learned particle level distribution automatically preserves quantities like momentum or charge~\cite{Bellagente2020InvertibleAgain, Ackerschott2023ReturningBelong}.
        %
        ...
        %
        If the Simulation has zero or very little support in some region that the Data populates, no amount of reweighting or network fitting can compensate; as discussed, this can be both a feature or a bug.

    \subsection{Regularisation in a RAN.}
    \label{subsec:regularisation-in-ran}
        ...
        %
        Unlike \textsc{OmniFold}, which iteratively trains many separate classifiers, RAN solves for all weights in a single training to convergence by jointly optimising $g$ and $d$.
        %
        ...
        \subsubsection{Wasserstein GAN}
            ...
            %
            Such losses correspond to minimising a divergence like the Kullback--Leibler divergence or the Jensen--Shannon divergence; if the real and model distributions do not overlap, the gradient of these losses vanishes, leading to unstable or mode collapsed solutions.
            
            By contrast, a RAN leverages the Wasserstein distance as the measure of difference between distributions.
            %
            The WGAN critic $d(x)$ does not output a class label but rather a real valued score, which should be high for real data and low for (in a RAN's case, reweighted) simulation.
            %
            The generator seeks to maximise this critic score on its output, thereby minimising the Wasserstein distance between the weighted Simulation and the Data distributions.
            %
            The objective function adopted in training a RAN can be written as
            \begin{align}
                \label{eq:ran_wgan_loss}
                L[g,d] = \E_{x\sim p}\qty[d(x)] - \E_{(z,x)\sim q}\qty[g(z;\,\vb*\beta)\,d(x)]+\lambda\,\E\qty[\qty(\norm{\grad_{\hat{x}}d(\hat{x})} - 1)^2]\;,
            \end{align}
            where $x\sim p$ denotes a detector level event from the Data, and $(z,x)\sim q$ indicates a paired MC event with Generation features $z$ and corresponding Simulation observation $x$.
            %
            The MC is generative, so $z\to x$ are always paired via the detector model.
            ...
            The minimax objective on the loss \cref{eq:ran_wgan_loss} with respect to $g$ and $d$ corresponds to the following adversarial optimisation:
            %
            \begin{enumerate}
                \item $d$ tries to maximise $L_{\text{WGAN}}$ by making the Data--Simulation discrepancy as large as possible by assigning higher scores to real Data than to weighted Simulation samples.
                \item $g$ tries to minimise it to make the reweighted Simulation look as similar as possible to Data by adjusting weights $g(z;\,\vb*\beta)$.
            \end{enumerate}

\begin{figure}
        \centering
          \resizebox{\linewidth}{!}{\input{figures/chapter-06/schematicV2}}
        \caption[Schematic description of the training setup of a RAN.]{The training setup of a RAN.
        %
        $g$ generates weights to reweight the Generation. The Simulation dataset inherits its weights from the matching Generation events.
        %
        The detector emulation is only run once, since a reweighted Simulation is created via importance weights and not by changing the features themselves.
        %
        The critic $d$ learns to classify events between Data and reweighted Simulation.
        %
        When the discriminator is maximally confounded, the reweighting is successful. }
        \label{fig:ran-schematic}
    \end{figure}
            ...
            %
            This setup is schematically illustrated in \cref{fig:ran-schematic}
            ...
            An additional benefit is that a RAN can, in fact, handle cases with minimal detector level overlap between Simulation and Data.
            %
            Since the Wasserstein metric remains finite as long as the support of the weighted simulation can be transported to cover data, even if initially disjoint, RAN does not strictly require the Simulation and Data distributions to overlap at all.
            %
            Of course, overlap at particle level is still required.
            %
            RAN cannot invent Truth events outside the Generation’s support.
            %
        ...
        \subsubsection{Regularising the critic: Lipschitz constraints}
            ...
            The critic network $d(x)$ in a RAN is subject to the \(1-\)Lipschitz constraint required by the Wasserstein theory.
            %
            This  is enforced through two complementary regularisation techniques.
            %
            The first, already noted, is the \textbf{gradient penalty} term in \cref{eq:ran_wgan_loss}.
            %
            Rather than clipping weights\footnote{The original WGAN approach,~\cite{arjovsky_wasserstein_2017} which can impede learning by reducing capacity.}, RANs adopt the improved strategy of penalising the norm of the critic’s gradient on randomly sampled interpolations between Data and Simulation to be close to 1.
            %
            ...
            The second technique used to regularise the RAN's training is \textbf{spectral normalisation} applied to the critic’s layers~\cite{miyato_spectral_2018}.
            %
            ...
            %
            Even small violations of the Lipschitz condition can lead to training instabilities, since the critic might exploit them to push $L_{\text{WGAN}}$ lower, causing $g$ to react with wildly large weights, making it essential that the Lipschitz constraint be observed strictly and carefully.
            ...
            %
            In effect, spectral normalisation places an upper bound on the influence of any individual Data–-Simulation discrepancy on the loss.
            %
            The combined use of gradient penalties and spectral normalisation in RANs was found to be effective for training stability---they keep the critic in check so that the adversarial training remains in a regime where gradients are informative and the generator’s updates remain moderate.
            %
            With these in place, a RAN substantially avoids the divergence and mode collapse issues that plague naive adversarial training~\cite{goodfellow_nips_2017, wiatrak_stabilizing_2020, Qian2021SelfNetworks, chenCDEGANCooperativeDual2021}.

        \subsubsection{Regularising the generator: initialisation and activation constraints.}
            On the generator side, a RAN incorporates two important measures to regularise the solution.
            %
            First, $g(z;\,\vb*\beta)$ is initialised close to the identity function before beginning adversarial training.
            %
            In the context of unfolding, the `identity' reweighting corresponds to assigning weight 1 to every MC event, i.e. initially assuming the Generation is the Truth and no reweighting is needed..
            %
            This initialisation reflects the belief that the generator only needs to make relatively small, smooth adjustments to the MC, which is a reasonable assumption in many cases where the Monte Carlo is approximate reality reasonably well.
            
            By starting close to the identity mapping, a RAN avoids a situation where early in training the critic finds large differences and drives $g$ to extreme compensations.
            %
            ...
            %
            Indeed, from the perspective of classical regularisation, a RAN's weight initialisation is equivalent to choosing the MC as a prior and initially penalising deviations from it.
            %
            ...
            %
            It is essential that $g(z)$ produce positive weights because events can only be reweighted by positive factors.
            %
            It is also essential that the generator have the capacity to yield a wide range of values  because some events might need weight $\gg 1$ if data are more abundant there, others weight $<1$ if data are scarce. 
            ...
            \[
                g(z) = \frac1P\ln(1 + \operatorname{softplus}(z)),
            \]
            where $\operatorname{softplus}(x) = \ln(1+e^x)$ is a smooth approximation to a ReLU which grows linearly for large $x$, but is smooth and non-zero for all real-valued inputs~\cite{dugas_incorporating_2000, glorot_deep_2011}.
            %
            Wrapping softplus in $\log(1+\cdot)$ further slows the growth for large inputs: as $x \to \infty$, $\mathrm{softplus}(x)\sim x$, so $\log(1+\operatorname{softplus}(x)) \sim \log(1+x) \sim \log x$.
...
This design acts as an adaptive regulariser, automatically preventing the numerical instabilities of exponential or even linear parametrisations while avoiding the expressivity limitations of bounded functions.
            ...
            The key advantage of this activation is worth re-iterating.
            %
            For even modest positive inputs, where the natural choice of the exponential would produce weights in the millions or billions,\footnote{\(\exp(13)\sim\num{d6},\; \exp(20)\sim\num{d9}\)} a RAN's $\log(1 + \text{softplus})$ activation yields much more manageable weights.\footnote{\(\log(1 + \text{softplus(13)}),\;\sim 2.5, \log(1 + \text{softplus(20)})\sim 3\).} This compression of the output range prevents the loss function dynamics from experiencing numerical instability while preserving the relative ordering of weights.
            %
            The activation thus acts as an adaptive regulariser, automatically preventing the numerical instabilities that plague naive exponential parametrisations without requiring explicit weight clipping or normalisation.
            


            \begin{figure}
                \centering
                \includegraphics[width=.85\linewidth]{figures/chapter-06/activations.pdf}
\caption[Comparison of activation functions to parametrise a RAN's generator.]{\small Comparison of activation functions considered for parametrising the generator. The main plot shows five functions on a log scale: log--softplus (blue solid), exp (orange dots), ReLU (green dash--dot), softplus (red dashed), and sigmoid (violet waved).
%
The inset has linear scaling. Exp exhibits rapid growth for moderately large inputs. The sigmoid saturates at 1, preventing large weights when needed. ReLU exhibits linear growth, but returns null gradients for negative inputs. Softplus also exhibits linear growth, but is a smooth function with non-zero gradients.
%
 log--softplus preserves smoothness, positivity, and positive gradients, with exp--like behaviour near zero to logarithmic growth for large $z$.
}
            ...
            \begin{enumerate}
                ...
                %
                This ensures that a RAN does not limit the solution space unduly, 
                \item Extremely large weights are disfavoured.
            \end{enumerate}
            ...
            %
           Since this is a soft constraint, large weights are not forbidden, and can be realised if the data demand them.
            %
        ...
        \subsubsection{Regularising with Generation}
            Finally, it is worth noting an intrinsic form of regularisation in a RAN that the architecture includes, `for free'.
            %
            By construction, the unfolded result is expressed as a reweighted version of the initial Generation sample.
            %
            This means the unfolded distribution cannot introduce structures that were not present (even in latent form) in the MC.
            %
            In effect, the Generation provides a prior support and shape for the solution.
            %
            If the true underlying distribution has features outside the Generation’s support, RANs, like any reweighting method, cannot recover them.
            %
            But conversely, RANs will not produce spurious artifacts that violate known physics encoded in the MC.
            ...
            %
            The design choice of `modulating a known density rather than learning a new one from scratch' is motivated by the desire to leverage this built--in form regularisation.
            %
            This allows a RAN’s solutions to be more statistically robust than those from unconstrained generative models, precisely because $g(z)$ operates within the scaffolding of the Monte Carlo sample.
            %
            This comes at the cost of some bias if the Monte Carlo is poor, which no method can escape without additional external input, but is a conscious regularisation choice favouring stability over unrestricted flexibility.
    ...
    In summary, the RAN methodology interweaves modern ML techniques with classical regularisation principles to achieve a stable unfolding.
        %
    The use of the Wasserstein distance provides a more flexible way of comparing distributions that avoids the brittleness of binary classification metrics.
        %
     Imposing Lipschitz continuity on the critic via gradient penalties and spectral normalisation keeps the adversarial game well behaved and prevents the discriminator from overemphasising statistical noise.
    %
    On the generator side, starting from the Generation and limiting the capacity for extreme weights through initialisation and activation choices anchor the solution close to expected physics and discourage overfitting.
    %
    Together, these innovations allow RANs to unfold full distributions in one shot, non-iteratively, without the severe instabilities that one might fear from an unconstrained GAN approach.
    
    In effect, the RAN algorithm attains a balance.
    %
    ...
    %
    The result is an unfolding method that achieves competitive performance to iterative methods like \textsc{OmniFold}, while operating in a single training pass and maintaining controlled behaviour even in challenging regimes\footnote{Such as limited detector overlap or regions with low statistics.}.
    %
    ...
    %
    It marries the strengths of adversarial learning with careful regularisation choices, yielding a novel and powerful approach to this classic problem.
...
\sectionminitoc
    \subsection{Neural network architecture.}
        ...
        %
        Each hidden layer uses a Leaky Rectified Linear Unit (Leaky ReLU) activation~\cite{Maas2013RectifierModels}, defined in \cref{eq:leaky-relu}, with slope parameter \(\alpha = 0.3.\)
        ...
        is applied to \(t\).
         ...
         Finally, the weights ${w_i}$ in each mini--batch are normalised so that their average is unity.
         %
        ...
        The critic $d(x;\,\boldsymbol{\theta})$ is a discriminator--like network that takes as input the detector level feature vector $x \in \mathbb{R}^{N_D}$ of an event, where \(N_D\) is the dimension of Data and Simulation, and outputs a real valued score.
        %
        ...
        %
        As discussed, spectral normalisation scales the layer weights such that their largest singular value is 1, effectively controlling the Lipschitz constant of $d$ without weight clipping.
        %
        ...
        %
        Additionally, a mild output clipping\footnote{Not weight clipping.} is applied to the critic scores, capping $d(x)$ within a reasonable range to eliminate spurious large values that could arise from statistical fluctuations, which in this context would correspond to unphysical negative probabilities or overly large separation scores~\cite{dai_rethinking_2022, arjovsky_wasserstein_2017}.
        %
        A schematic of architecture implemented is shown in \cref{fig:model-arch}.
        
\begin{figure}
    \centering
    \resizebox{0.75\linewidth}{!}{\input{figures/chapter-06/model-arch}}
    \caption[RAN architecture with generator and critic networks]{\small Architecture of a RAN. 
    \textbf{Left:} The generator processes particle level features through three hidden layers (50 nodes each) with LeakyReLU activation, batch normalisation, and 20\% dropout. The output layer produces event weights using the log--softplus activation.
    \textbf{Right:} The critic evaluates detector level features through spectrally normalised dense layers with LeakyReLU activation, layer normalisation, and dropout, outputting a scalar score.
    %
    The networks are trained adversarially using a Wasserstein loss with gradient penalty, where the critic learns to distinguish weighted Simulation from Data while the generator learns to reweight Generation to induce weights on Simulation minimise this distinction.}
...
\end{figure}
    ...
    \subsection{Adversarial training procedure.}
        ...
        %
        The studies presented below used an update ratio of 3:2 for the critic:generator, meaning that the critic is updated slightly more frequently to keep it near optimality as the generator evolves.
        %
...
\begin{algorithm}
    ...
    \algospacing
    \Require{
      Data $\sim p(x)$; Generation, Simulation pairs \(\sim q(z, x)\),\\
      critic $d_{\theta}$, generator $g_\beta$,\\
      gradient penalty weight $\lambda$,\\
      critic steps $n_d$, generator steps $n_g$, batch size $B$.
    }
    \Ensure{Trained parameters $\theta$ and $\beta$}
    \For{iteration $=1,2,\dots$}
      \State \Comment{Critic updates}
      \For{\(i\) in \(1\dots n_d\)}
        ...
        \]
        \State $\theta \leftarrow\theta + \eta_d\,\grad_{\theta}L_d$    
      \EndFor
    \EndFor
    \algstore{wgangp}
    \end{algorithmic}
...
\begin{algorithm}\ContinuedFloat
    ...
    \algrestore{wgangp}
    \For{iteration $=1,2,\dots$}
      \State \Comment{Generator updates}
      \For{\(j\) in \(1\dots n_g\)}
        ...
        \[
          ...
          = -\frac1B\sum_{j=1}^B w_j\,d_{\theta}\bigl(x_j^{\text{sim}}\bigr)
            + \frac1B\sum_{i=1}^B d_{\theta}(x_i^{\text{data}})
        \]
        \State $\beta \leftarrow\beta - \eta_g\,\grad_{\beta}L_g$
      \EndFor
    \EndFor
    \end{algorithmic}
\end{algorithm}
        ...
        %
        Both networks are optimised using the RMSProp optimiser, which was found to outperform \textsc{Adam} in the WGAN framework, consistent with previous studies~\cite{henderson_where_2018}.
        %
        The learning rate for both generator and critic are set in the range $\eta \sim 1\times10^{-4}$ to $5\times10^{-4}$, with the exact value tuned by dataset.
        %
        ...
        %
        In practice, the performance of the RAN method is found to be robust against moderate changes in these settings: for instance, altering the learning rate by a factor of two or using 40 or 60 nodes per layer, instead of 50, had no significant impact on the final unfolded distributions.
        %
        ...
        Throughout training, key indicators should be continuously monitored to ensure the optimisation remains stable.
        %
    ...
    \subsection{Datasets and preprocessing.}
        ...
        \subsubsection{Gaussian example.}
            ...
            %
            Generate two sets of events from normal distributions at the particle level, a Truth distribution $Z_{T} \sim \mathcal{N}(\mu_{T},\,1)$ and a Generation distribution $Z_{\text{G}} \sim \mathcal{N}(\mu_{G},\,1)$, with means $\mu_{G}=0$ and $\mu_{G}=-1$.
            %
            Sample \(\num{d4}\) events from the Truth distribution and \(\num{d5}\) events from the generation distribution.
            
            In lieu of detector effects, each particle level value $z$ is transformed by a simple deterministic response: $x = S \cdot z$, where $S$ is a scalar `smearing' factor which is constant for any given individual unfolding experiment.
            %
            ...
            %
            No additional preprocessing of the input features is necessary since they are already bounded and standardised aside from mean shifts.
            ...
            %
            In the limit of infinite statistics, the optimal weight would be proportional to the ratio of Truth to Generation densities at $z$, which in this case is
            \begin{align}
                g_* &= \exp[-\frac12(z-\mu_{T})^2 + \frac12(z-\mu_{G})^2]\\
                    &=\exp(z + \frac12).
            \end{align}
            
            This example also allows one to verify that a RAN’s adversarial training indeed converges to the correct solution, by comparing the unfolded distribution to the analytic expectation.
        ...
        \subsubsection{Jet substructure dataset.}
            ...
            %
            This experiment follows the setup of the \textsc{OmniFold} study described in~\cite{andreassen_omnifold_2020} to enable direct comparisons.
            ...
            %
            Each event in both samples contains a high\(-p_T\;Z\) boson with $p_T^Z > \siqty{200}{\GeV}$ decaying leptonically, produced back to back with a hadronic jet.
            %
            ...
            %
            For each jet, this study considers six different substructure observables that characterise its internal pattern of particles.
            %
            These include classic measures like the jet mass $m$ and jet width $w$, as well as more specialised variables such as the \(N-\)subjettiness ratio $\tau_{21}$, the groomed momentum fraction $z_g$ from the Soft Drop algorithm, and a logarithmic version of the dimensionless jet mass $\ln\rho$.
            %
            ...
            During training, the \textsc{Delphes+Herwig} jets are treated as the Data distribution $p(x)$ and the \textsc{Delphes+Pythia} jets as the initial Simulation, $q(x)$.
            %
            At each iteration, mini--batches of size $B=256$ jets are drawn from each distribution.
            %
            Given the relatively large size of the samples, one need not worry about cycling through the data too many times and can simply train for a fixed number of epochs determined by early stopping.
            %
            One must ensure that each jet in the training sample is used many times over the course of training, which is important for the generator to refine the weights on all regions of phase space.
            %
            Once training is complete, the learned weights $w = g(z)$ can be applied directly to the Generation events to produce an unfolded distribution, which can then be compared to the Truth, i.e. \textsc{Herwig} particle level distribution.
            ...
            %
            This is a common practice in validation studies, allowing one to quantitatively evaluate the accuracy of the unfolding by comparing to known truth.
            %
    ...
    \subsection{Training, monitoring and validation.}
        ...
        First, one should partition each dataset into a training set used to update network parameters, a smaller validation set, used for performance monitoring during the training, and a testing set.
        %
        The ratios used were \(\text{training : validation : testing} = 70:15:15.\)
        %
        The validation data is not seen by the networks during gradient updates, but one computes the same losses on it to check how well the model generalises.
        %
        Specifically, after every fixed number of training epochs, one evaluates the critic loss $L_d$ on the validation batch and tracks its evolution.
        ...
        For instance, in the Gaussian example with moderate smearing, one observes the VLC divergence between the weighted Generation and Truth decrease rapidly in the first few dozen epochs and then flatten out at a low value, indicating that the generator had found an effective weighting scheme.
        %
        ...
        %
        By the end of training, the unfolded distributions for all six jet observables are nearly indistinguishable from the truth, as quantified by final Wasserstein distances on the order of $10^{-2}$ and VLC divergence on the order of $10^{-3}$.
        %
        \cref{tab:comp-w,tab:comp-vlc} are included in \cref{sec:ran-results} comparing these divergence metrics for RAN versus other methods; the RAN implementation achieves competitive or superior scores, validating the training procedure described.\footnote{See~\cite{andreassen_omnifold_2020} for comparable benchmarks involving iterative methods.}

        The monitoring process also guards against the possibility of divergence or weight blow up.
        %
        If at any point the training appears to destabilise,\footnote{E.g., the critic loss oscillates wildly or the weights $\qty{g(z)}$ start to become extremely large for a small subset of events.} one can intervene by stopping early and/or introducing additional regularisation.
        %
        In practice, due to the regularisation implemented (spectral norm, gradient penalty, dropout), such pathological behaviour was rare.
        ...
        %
        When there are regions that have little MC density but non-negligible data density, any reweighting based method will assign large weights to the few simulation events in that region, and a RAN is no exception.
        %
        Nonetheless, the spectral normalisation on the critic help limit how fast and how large those weights could grow.
        %
        If needed, one could impose an explicit cap on weights or an additional \(L_p\) norm term in the loss to penalise overly large weights, but this was not found to be necessary for these benchmarks.

    In summary, the ML implementation of a 3RAN described above proves to be robust and reproducible across the different datasets.
    %
...
\section{Results}
    ...
    This section evaluates Reweighting Adversarial Networks on the two case studies described in the preceding section.
    %
    In each case, RANs' performance is compared to two baseline methods, the \textsc{OmniFold} algorithm and IBU.
    %
    Quantitative divergence metrics are reported for each method that measure the algorithms' ability to reproduce the Truth distributions, as well as the methods’ stability to statistical challenges.

    \subsection{Gaussian model with smearing.}
        This study represents a targeted unfolding scenario to stress test RANs in a regime of limited detector level overlap.
        %
        ...
        %
        In contrast, RANs perform reweighting at particle level and use an optimal transport based loss to compare detector level outputs.
        %
        One therefore expects a RAN to be more robust in scenarios with poor detector level overlap, provided the particle level distributions still cover each other.

        To quantify performance the Vincze–-Le Cam (VLC) divergence $\Delta_{VLC}$ between the unfolded outcome and the true distribution is used as a metric of accuracy.
        %
        \cref{fig:omnifold-comp} summarises the unfolding accuracy as a function of the smearing factor, with $\Delta_{\text{VLC}}$ plotted for RANs and \textsc{OmniFold}.

        At minimal smearing, both methods perform excellently, and \textsc{OmniFold} achieves a slightly smaller divergence to truth than the RAN.
        %
        For example, at smearing $S = 1, \Delta_{\text{VLC}}$ for \textsc{OmniFold} is $0.057\times10^{-3}$, compared to the RAN’s $0.122\times10^{-3}$.
        %
        ...
        %
        In stark contrast, RANs maintain stable performance across the entire range of smearings.
        %
        As shown in \cref{fig:omnifold-comp}, RANs' $\Delta_{\text{VLC}}$ remains approximately constant, with only slight variation within statistical error, as smearing increases.
        %
        Even at the largest smearing factors, the RAN's divergence with Truth is essentially the same as it was at zero smearing, indicating that the RAN successfully unfolds the distribution despite separation in detector level distributions.
        %
        This behaviour can be attributed to the RAN's methodological property of only reweighting at particle level, combined with its use of an optimal transport loss at detector level, which, unlike a classifier, does not require overlapping support to provide a meaningful gradient---the adversarial training can adjust weights to minimize the Wasserstein distance between the smeared Simulation and Data, even if those distributions are partially disjoint.
        ...
        %
        RANs' results are consistent across bootstrap resamples while \textsc{OmniFold}’s uncertainty grows with smearing.
        %
        In summary, this Gaussian study demonstrates that a RAN is robust to limited detector level support, recovering the true distribution reliably in a single pass, whereas \textsc{OmniFold} requires sufficient detector level overlap to perform optimally.
        %
        This advantage of RANs becomes valuable in scenarios where detector effects can be large.
        
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figures/chapter-06/OF_delta.pdf}
    \caption[Unfolding performance comparison between RAN and OmniFold under increasing detector smearing]{Comparison of unfolding performance under increasing detector smearing. 
    The $\Delta_{\text{VLC}}$ between unfolded and true distributions is shown as a function of the smearing factor for a RAN (black circles) and OmniFold (red triangles). 
    While OmniFold achieves slightly better accuracy at minimal smearing, its performance degrades as smearing increases, reflecting its reliance on detector level overlap. 
    In contrast, the RAN maintains stable performance across all smearing levels, demonstrating robustness to limited detector level support. 
    Generation (yellow diamonds) is shown for reference. 
...
\end{figure}
    ...
    \subsection{Jet substructure unfolding results.}
        This study assesses RANs on a realistic high energy physics unfolding task involving correcting jet substructure observables from detector level Data (simulated) to particle level Truth.
        %
        ...
        %
        The observables chosen span a range of distribution shapes---from broad distributions to sharply peaked ones and those with kinematic cut--offs---providing a comprehensive test bed.
        %
        The RAN method is applied to learn a single weighting function $g(z)$ that assigns a single weight to each six dimensional Generation event based on its features to achieve this alignment between Data and reweighted Simulation, as described in \cref{sec:ran-methodology-and-regularisation,sec:ran-ml-implementation}.
...
\cref{fig:particle-level-distribution} presents the unfolded particle level distributions for each of the six jet observables for the RAN, IBU, and \textsc{OmniFold}, compared to the Truth distributions.
            %
            Each panel shows the truth spectrum (\textsc{Herwig}, treated as `Truth') in filled blue, the generation in filled orange, the unfolded result from the RAN as a black solid line, the unfolded result from \textsc{OmniFold} as a red dashed line and the result from IBU as a green dotted line.
            %
            The ratio sub--panels below each panel display the unfolded/Truth ratio for each method, indicating how well each method approximates Truth across the range of each observable.
            %
            Both the RAN and \textsc{OmniFold} achieve good agreement with truth across all observables, a non-trivial accomplishment given the complexity of the distributions, and the RAN consistently provides a close match, particularly in regions that are challenging to unfold.

            For example, in the jet mass distribution in \cref{fig:particle-level-distribution}'s, top-left panel, the Generation initially underestimates the probability in the high mass tail and misplaces the peak relative to Truth.
            %
            After unfolding, the RAN’s distribution closely follows the truth curve: it accurately reproduces the peak around $m\approx 20$ GeV and the long tail up to high masses.
            %
            \textsc{OmniFold} also significantly improves the agreement, but its ratio plot reveals a slight residual bias in the tail, with deviations of order 5–10\% from unity in the highest mass bins, whereas the RAN’s ratio is within a few percent of unity throughout.
            
\begin{landscape}
\begin{figure}
    \centering
    \includegraphics[width = 0.97\textwidth]{figures/chapter-06/six_var_with_ratio.pdf}
    \caption[Unfolded particle level distributions for six jet substructure observables, comparing RAN, \textsc{OmniFold}, and IBU.]{\small Particle level distributions of six jet substructure observables after unfolding. 
    %
    Each panel shows Truth as the blue filled histogram, Generation as the orange filled histogram, and unfolded results from the RAN (black solid), \textsc{OmniFold} (red dashed), and IBU (green dotted). 
    %
    The observables are jet mass $m$ and soft-drop mass $M$ (left column), constituent multiplicity and jet width $\tau_1$ (middle column) and $N$-subjettiness ratio $\tau_{21}$ and groomed momentum fraction $\ln\rho$ (right column).
    %
    The sub--panel under each panel shows the ratio of each unfolded result to truth, demonstrating that the RAN achieves close agreement across all observables, particularly in challenging regions such as distribution tails.
    %
...
\end{landscape}
            ...
            %
            The Generation initially produces a broader $\tau_{21}$ distribution than Truth (\textsc{Herwig}), indicating an overestimation of two--prong substructure.
            %
            ...
            %
            The RAN handles this boundary reasonably well.
            %
            The unfolded $z_g$ distribution from the RAN matches the truth both in the low $z_g$ region and near the cut-off, correctly recovering the falling slope as $z_g \to 0.5^-$.
            %
            ...
            %
            The RAN succeeds in reducing the discrepancy at both the low-$M$ and high-$M$ extremes, yielding an unfolded distribution that tracks the truth across the full range.
            %
            ...
            %
            In summary, qualitatively, the RAN’s unfolded spectra are virtually indistinguishable from the truth for all six observables, within statistical uncertainties,
...
\input{tables/6-4}
            ...
            One also observes that IBU, a binned, per--observable method, generally underperforms the unbinned methods here.
            %
            ...
            %
            In contrast, a RAN and \textsc{OmniFold} operate on unbinned data and can leverage the full event information, yielding superior precision.
            
            It is also noteworthy that the RAN’s advantage is achieved while producing a single set of event level weights that simultaneously correct all six distributions.
            %
