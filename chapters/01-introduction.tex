...
\sectionminitoc
    ...
    %
    In high energy physics, the choice of units reflects not simply an arbitrary choice, but a philosophical stance about the nature of reality.
    %
...
\subsection{Coordinate systems and relativistic geometry.}
    ...
    \begin{definition}[Pseudorapidity]
    ...
    %
    As expected, the massless case ($\nicefrac m{p_T}=0$) lies exactly on the diagonal $y=\eta$, while non-zero $\nicefrac{m}{p_T}$ introduces a systematic deviation that grows at large $|\eta|$.
    %
    Even a modest ratio ($\nicefrac m{p_T}=0.1$) produces a measurable shift, and by $\nicefrac{m}{p_T}=2.0$ the rapidity is significantly reduced relative to $\eta$.
    %
...
\begin{definition}[Angular distance]
...
\end{definition}
    ...
    \subsection{Some common sets and groups.}
    The standard notation adopted for frequently occurring sets and groups is listed in \cref{tab:notation}.
    %
    \input{tables/1-2}
    While these conventions are widely used in the mathematical literature, they are listed here to avoid ambiguity and provide a convenient reference.
    
    \subsection{Statistical frameworks and uncertainty quantification.}

        \begin{note*} Some basic notation:
            \begin{itemize}
                \item \(\P(\cdot)\) denotes the probability of an event.
                \item \(\E_{p}[\cdot]\) denotes the expected value of a random variable under probability density function \(p.\) The probability density function may be omitted if it is clear from context.
            \end{itemize}
        \end{note*}
    ...
    \begin{definition}[Profile likelihood ratio]
...
\subsection{Datasets.}
    ...
    \begin{figure}
    \centering
    \resizebox{0.7\linewidth}{!}{\input{figures/chapter-01/dataset-taxonomy.tex}}
...
\cref{fig:dataset-taxonomy} illustrates this taxonomy graphically.
    ...
    \subsubsection{Four dataset categories.}
        ...
        \begin{description}
            \item[Truth\(\,\sim p(z)\)]  represents nature at the particle level---the actual physical distributions HEP experiments ultimately seek to measure.
            %
            ...
            \item [Data\(\,\sim p(x)\)] comprises natural events after detector effects.
            %
            ...
            \item [Generation\(\,\sim q(z)\)] comprises Monte Carlo events at the particle level, providing a controlled means to understanding theoretical predictions.
            %
            ...
            \item [Simulation\(\,\sim q(x)\)] represents Monte Carlo events processed through detailed detector modelling, creating artificial detector level datasets with known particle level origins.
            %
    ...
    \subsubsection{The detector response.}
        ...
        %
        For simulated events, the detector simulation models these processes through programs like \textsc{Geant4}~\cite{allison_recent_2016,Allison:2006ve,GEANT4:2002zbu} that track particles through detailed detector geometries, applying the current best understanding of particle matter interactions.
...
Their use dates all the way back to Rutherford's scattering experiments that revealed the existence of atomic nuclei by analysing angular distributions of scattered alpha particles \cite{F.R.S.1911LXXIX.Atom}.
    ...
    \label{subsec:exp_measurement}
        ...
        \begin{definition}[Integrated luminosity]
    ...
    \subsection{Applications in particle physics.}
        ...
        \subsubsection{Monte Carlo tuning.}
            \begin{definition}[Monte Carlo tuning]
            ...
            \begin{definition}[Parton distribution functions]
        ...
        \subsubsection{Parton luminosity.}
            \begin{definition}[Parton luminosity]
        ...
        \subsubsection{Detector performance validation.}
            ...
            \subsubsection{Future planning.}
                ...
                %
                For example, the surprisingly large Higgs production cross section at the LHC, for instance, has already influenced design considerations for future electron--positron Higgs factories.           
...
%
All of the original research presented in this dissertation represents collaborative efforts with many brilliant scientists, to each of whom I am deeply grateful.
%
The views I present here my personal perspective on these developments.
%
Where I discuss or present specific results primarily generated by my collaborators, I explicitly indicate this attribution. The collaborative nature of this work has been one of its greatest strengths, and I am indebted to all those who contributed their expertise and insights to these projects.
