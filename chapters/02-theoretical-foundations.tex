...
\subsection{Likelihood-Based Formulation}
    ...
    \label{subsec:challenges-in-high-dimensional-phase-spaces}
        ...
        \label{subsubsec:binned-methods}
            ...
            %
            The process of discretizing continuous distributions into finite bins introduces artificial boundaries  onto the underlying physics.
            ...
            %
            These include additional kinematic variables not captured in the chosen binning scheme, event--level properties such as particle multiplicity and missing energy, detector conditions like instantaneous luminosity and pile-up activity, and correlations with other particles produced in the same collision.
            %
            ...
            %
            By marginalizing over these features, binned methods introduce systematic biases that can propagate through the unfolding procedure and distort the final measurements in ways that are difficult to quantify or correct.
        ...
        Rather than explicitly modelling the response function r(x|z), modern techniques like OmniFold~\cite{andreassen_omnifold_2020} employ iterative reweighting strategies that learn implicit mappings between truth and detector-level distributions.
        %
...
\section{Unbinned Methods: Statistical Considerations}  
    ...
    \subsection{Principles and Implementations}
        ...
        \subsubsection{Reweighting Methods}
            The most established approach in this category is OmniFold, which employs an iterative procedure inspired by the Iterative Bayesian Unfolding (IBU) algorithm.
            %
            OmniFold alternates between two steps that estimate likelihood ratios using binary classifiers.
            
            In each iteration \(k\), OmniFold first trains a classifier to distinguish between data and simulated detector--level events, referred to as \emph{simulation} (Sim.), yielding the ratio
            \begin{equation}
...
\section{Evaluation metrics for unfolding}
    ...
    \subsection{Statistical Metrics for Evaluating Point Estimates}
        ...
        \subsubsection{Pull distributions}
            ...
            %
            Since pull is defined as the ratio of the bias to the estimated uncertainty, for a properly calibrated unfolding method, two conditions must be satisfied simultaneously.
            %
    ...
    \subsection{Evaluation of correlation structure}
        ...
        \subsubsection{Event--to--event correlation metrics}
            ...
            \begin{figure}
                ...
                \caption[Weight correlation between event pairs as a function of distance between events.]{Average weight correlation between two events as a function of the absolute distance between the events in the observable for Gaussian data unfolded using OmniFold~\cite{Desai:2025mpy}.\footnote{Figure created by Owen Long.}
                }
                \label{fig:corr-decay}
            \end{figure}
    \subsection{Method specific evaluation metrics}
        \subsubsection{Iterative methods}
            For iterative methods like Iterative Bayesian Unfolding (IBU) or OmniFold, convergence behaviour provides important diagnostic information.
        %
