...
\subsection{Likelihood based formulation.}
    ...
    \begin{note*}
        The likelihood function in \cref{eq:likelihood-function} represents the probability of the data \(\mathbf{X}\) given the parameters \(\theta\) for relative cross sections.
        %
        If one were to use the likelihood function for absolute cross sections, the likelihood would also depend on the luminosity, and therefore include a poisson term for \(N\).
    \end{note*}
    ...
    \label{subsec:challenges-in-high-dimensional-phase-spaces}
        ...
        \label{subsubsec:binned-methods}
            ...
            \begin{theorem}[Scaling of relative uncertainty]
                \label{thm:relative-uncertainty}
...
\label{sec:ill-posed}
    ...
    \item[TUnfold~\cite{schmitt_tunfold_2012}] formulates unfolding as a least squares problem with Tikho\-nov regulari\-sation to penalise large second derivatives, preserving smooth\-ness.
    \end{description}
...
\label{sec:theoretical-foundations.unbinned-methods}
\sectionminitoc
    ...
    \subsection{Principles and implementations.}
        ...
        \label{subsubsec:principles-and-implementation.reweighting-based-methods}
...
\sectionminitoc
    ...
    \end{definition}
        ...
        \cref{tab:dist_metrics} summarises the various distributional metrics and their relative strengths for unfolding evaluation.
        \input{tables/2-2}
        ...
        \label{subsubsec:pull-distributions}
            ...
            \begin{definition}[Pull distribution.]
            \label{def:pull-distribution}
        ...
        \end{definition}
            ...
            %
            This is formalised in \cref{thm:pull-normality}.

            \begin{theorem}[Asymptotic standard normality of pulls]
            \label{thm:pull-normality}
            ...
            \cref{thm:pull-normality} provides a rigorous theoretical foundation for the intuitive notion that a pull distribution with non-zero mean indicates systematic bias in the unfolding procedure. 
            %
            Deviations from unit standard deviation indicate either overestimation or underestimation of uncertainties.
            %
            The theorem also provides a diagnostic for the quality of the uncertainty estimates, as a pull distribution with non-zero mean or non-unit standard deviation indicates that the uncertainty estimates are not accurate.
    ...
    \subsection{Evaluation of correlation structure.}
        ...
        \subsubsection{Event--to--event correlation metrics.}
            ...
            \begin{equation}
                \label{eq:n-eff}
                N_{\text{eff}} = \frac{\left(\sum_{i} w_i\right)^2}{\sum_{i,j} w_i\,w_j\,\vb*{\rho}(w_i, w_j)}\;.
            \end{equation}
            ...
            The denominator can be decomposed into of two components, which together account for the increased variance introduced by weight correlations.
            %
            The first component, \(\sum w_i^2\), captures the variance contribution from individual event weights, similar to the standard effective sample size formula for independent weighted observations.
            %
            The second component, \(2\sum_{i<j} w_i \,w_j\, \vb*{\rho}(w_i, w_j)\), accounts for the covariance contributions between all pairs of events, where the factor of 2 arises because each pair appears only once in the sum over \(i<j\).

            When weights are uncorrelated, \(\vb*{\rho}(w_i, w_j) = 0,\) and the formula reduces to the familiar \(N_{\text{eff}}=\nicefrac{\qty(\sum w_i)^2}{\sum w_i^2}.\)
            %
            However, upon unfolding methods, nearby events in phase space often receive similar weight corrections\footnote{As will be discussed in greater detail in \cref{chap:unbinned_correlations}.}, leading to correlations that increase the denominator and reduce the effective sample size.
            %
            ...
            \[
                \sigma_S^2 = \sum_i \sum_j w_i\, w_j\, \Sigma_{ij},
            \]
             which is the denominator of the \cref{eq:n-eff}.
             %
             The effective sample size is then defined as the ratio of the squared expectation to the variance, providing a measure of statistical efficiency that accounts for both weight magnitudes and their correlations. Formally,
            ...
            If $\rho(w_i,w_j)=0$ for all $i\ne j$, the denominator reduces to $\sum_i w_i^2$ and $N_{\mathrm{eff}}$ simplifies to the familiar independent–weights expression $N_{\mathrm{eff}} = (\sum_i w_i)^2/\sum_i w_i^2$. In unfolding applications, nearby events in phase space often have $\rho(w_i,w_j)>0$, modifying the denominator of~\cref{eq:n-eff} and thus reducing $N_{\mathrm{eff}}$, which quantifies the corresponding change of statistical power.
            ...
            \begin{figure}
                ...
                \caption[Weight correlation between event pairs as a function of distance between events.]{Average weight correlation between two events as a function of the absolute distance between the events in the observable for Gaussian data unfolded using \textsc{OmniFold}.\protect\footnotemark
                }
    ...
    \subsection{Method specific evaluation metrics.}
        ...
        %
        To study the convergence behaviour, one plots metric values (e.g., \(\chi^2\) or NLL) as a function of iteration number.
        %
        One then compares unfolded distributions at different iterations to assess stability and analyse how the bias--variance trade-off evolves with iteration number.

        \subsubsection{Bayesian methods.}
            {\tolerance=1000
            For Bayesian unfolding methods such as Fully Bayesian Unfolding (FBU)~\cite{choudalakis_fully_2012} or \textsc{Neural Posterior Unfolding} (NPU)~\cite{acosta2024npu}, additional posterior specific metrics are relevant.\par}
            
            One can compare detector level data to detector level predictions generated from the posterior.
            %
            One assesses convergence using standard MCMC based diagnostics like the Gelman--Rubin statistic~\cite{Gelman:1992zz} or effective sample size.
            %
            The width of the posterior allows one to evaluate the posterior uncertainty in relation to the true frequentist variance.

    \subsection{Practical considerations.}
        In general, when comparing different unfolding methods, a structured evaluation framework ensures fair and comprehensive assessment. Such a framework should consider the following aspects:
        \begin{description}
            \item[Computational efficiency:] Measuring training time, inference time, and memory requirements.
            \item[Dimensionality scaling:] Assessing how performance metrics change as the dimensionality of the problem increases.
            \item[Prior dependence:] Evaluating robustness to different initial simulations.
            \item[Regularisation parameter sensitivity:] Comparing how performance varies with changes in regularisation strength.
        \end{description}
        ...
        %
        The two most commonly used techniques are cross validation, which involves splitting the detector level data, unfolding one portion, then forward folding it, and comparing predictions against the held out portion; and bootstrapping, where one generates multiple resampled datasets to assess the unfolding procedure.
        ...
        %
        The simplest kinds of closure tests involve applying detector simulation to a known particle level distribution, then unfolding the resulting detector level distribution and comparing with the original input.
        %
        ...
        %
        One can test the sensitivity of the method to systematic uncertainties by applying variations to the response matrix based on known systematic uncertainties and assessing the impact on unfolded distributions.
        %
        For methods that support nuisance parameter profiling, evaluating how effectively nuisance parameters are profiled out is a valuable test for the effectiveness of the method.

        Rigorous evaluation of unfolding methods requires a multifaceted approach that considers accuracy, uncertainty quantification, and computational performance.
        %
        ...
        Regardless of the method used, uncertainty calibration through pull distributions and correlation structure assessment are important to validate any measurement.
        %
