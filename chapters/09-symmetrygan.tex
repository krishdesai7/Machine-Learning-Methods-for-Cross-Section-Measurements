...
\section{Symmetries and unfolding.}
    \subsection{The complementary nature of symmetry discovery and unfolding.}
        ...
        %
        Unfolding endeavours to undo the convolution with the detector response and recover the true differential cross section or underlying distribution that generated the measurements.
        %
        ...
        %
        For example, architectures that conserve the 4--momentum or enforce Lorentz invariance by design, as discussed in \cref{chap:ml-for-unfolding}, effectively impose such constraints, narrowing the set of viable solutions.
        %
        ...
        %
        \cref{tab:unfolding-vs-symmetry} summarises the differences and points of contact between the two.
...
\input{tables/9-1}
    ...
    The research presented in this chapter builds upon the foundations laid in earlier chapters of this thesis, extending the paradigm of symmetry utilisation in the context of measurement and unfolding.
    %
    ...
    %
    The inclusion of symmetry constraints in unfolding models described in \cref{chap:npu,chap:moment-unfolding,chap:ran} through preserving physical invariants like momentum or charge conservation in the generative model could help reduce the solution space and lead to more physically plausible unfolded results.
    ...
    %
    In \cref{sec:improved-measurement-prediction} is a discussion of potential symmetry aware unfolding methodologies and how enforcing the symmetries identified by \textsc{SymmetryGAN} can improve the precision of differential cross section measurements.
    %
...
\sectionminitoc
    ...
    %
    \begin{definition}[Symmetry under a group.]
    \label{def:symmetry-under-a-group}
    ...
    \end{definition}
    \begin{definition}[Symmetry of a function]
    \label{def:symmetry-of-a-function}
    ...
    \begin{definition}[Symmetry of a probability density function]
    \label{def:symmetry-of a pdf}
    ...
    In measure theoretic language, a symmetry corresponds to the existence of an invariant measure.

    \begin{definition}[Invariant measure]
    \label{def:mu-symmetry}
    ...
    \]
    \end{definition}
    These definitions encompass both continuous symmetries\footnote{Lie groups, such as rotations depending on a continuous angle parameter.} and discrete symmetries.\footnote{Groups constructed as Jordan--H\"older extensions~\cite{solomon_brief_2001, HolderDieOrdnungszahlen, jordan_traite_nodate} e.g. a mirror reflection or a permutation of identical objects.}
    
    Symmetry principles lie at the heart of modern particle physics and strongly influence every aspect of the field.
    %
    ...
    %
    In the context of HEP experiments, many observables are governed by symmetries of the underlying theory as well as symmetries introduced by the detector and measurement process.
    %
    ...
    This section provides a rigorous overview of symmetries relevant to particle physics and measurements.
    %
    ...
    \label{subsec:hep-symmetries}
        ...
        %
        A given interaction process should yield identical outcomes whether the laboratory frame is, say, Earth bound or boosted to a constant velocity with respect to the Earth.
        %
        ...
        %
        Gauge symmetries are local symmetries that entail the introduction of gauge bosons.
        %
        ...
        %
        The most notable instance of this is the well known CP violation in neutral kaon and $B$-meson decays.
        %
        Violation of CP means those processes occur at different rates or with different phase relationships than their CP-mirrored counterparts~\cite{Neubert:1996qg}.
        %
        ...
        %
        The observed deviations are vital clues about the underlying physics.
        
        Parity by itself is also violated maximally in the weak interaction.\footnote{Classic examples are the left handed nature of neutrinos and the parity asymmetric angular distribution of electrons in polarised $^{60}$Co beta decay~\cite{Wu:1957my}.}
        %
...
\input{tables/9-2}
        ...
        %
        In quantum theories this is enforced by (anti)symmetrisation of identical particle states.
            %
...
\input{tables/9-3}
    ...
    \label{subsec:noisy-symmetries}
        ...
        %
        For instance, to confirm CP symmetry in production of particle vs antiparticle to the \num{e-3} level, one needs detector efficiencies known to better than \num{e-3} between positively and negatively charged particle detection~\cite{Gordon:2013eha}.
        ...
        \subsubsection{Dimensionality challenges.}
            ...
            %
            Hence traditional methods often restrict attention to physically motivated symmetry transformations.\footnote{Rotations, reflections, boosts, particle exchanges, etc.}
            ...
            %
            Conceptually, a \textsc{SymmetryGAN} trains a generator (applying candidate transformations) against a discriminator to test if the transformed data is statistically indistinguishable from the original data.
            %
            ...
            %
            The rigorous understanding of symmetry and symmetry--breaking provided in this section will form the foundation on which those computational methods build, ensuring that any discovered `symmetry' is physically meaningful and relevant to the challenges of unfolding and analysing HEP data.
...
\section{Statistical definition of dataset symmetries.}
    ...
    \subsection{Distinction between point and dataset symmetries.}
        ...
        %
        The distinction between symmetries of individual data elements and entire datasets, formalised in \cref{def:symmetry-under-a-group,def:symmetry-of-a-function,def:symmetry-of a pdf,def:mu-symmetry} lies at the heart of statistical theory of symmetries.
    ...
    \subsection{Inertial reference densities and their theoretical role.}
        ...
        \begin{definition}[Reference density]
        ...
        %    
        This can be analogised to the way in which phase space shifting operations that leave the Gibbs integration measure invariant can be understood as gauge transformations, with the reference density playing the role of a gauge fixing condition~\cite{Muller:2024rlq}.
        ...
        %
        Under a transformation \(g: Y \to X\), probability densities transform as
            \[
                p_Y(y) = p_X(g(y))\,|\det g'(y) |\;.
            \]
            
        This transformation law, involving the absolute value of the Jacobian determinant \(J = \det g'(y),\)~\cite{jacobi_determinantibus_1841} ensures probability conservation.
        %
        ...
        \begin{definition}[Diffeomorphism invariant probability densities.]
...
This invariance is strictly stronger than mere `point symmetry,' since it requires that the \emph{entire} probability measure be preserved under \emph{all} elements of $G$, compensating exactly for any local volume distortion. Furthermore,

\begin{definition}[Symmetric action of a group~\cite{luzzatto_chapter_2006}.]
...
\end{definition}
        ...
        Using the orbit--stabiliser theorem, one can decompose the action of such a group into orbits and stabilisers.
        %
...
\[
    ...
    \;=\; p(x).
\]
Thus every univariate random variable admits a $\Z_2$ `symmetry' of the form \cref{eq:quantile_symmetry}.
%
...
\subsubsection{Relative symmetry.}
    ...
    %
    \begin{definition}[Relative symmetry.]
...
%
Quantile based maps like the one described in \cref{eq:quantile_symmetry} will generally fail to preserve any fixed $p_I$ across different target densities.
...
%
The unimodular subgroup of the affine group $\operatorname{Aff}(p, q)$ that acts by linear maps with unit Jacobian determinant is the special affine group \(\ASL(p, q)\).
%
In fact, there is a canonical isomorphism between all such groups of the same dimension \(d = p+q\), and so these groups can be referred to simply as \(\ASL(d)\) independent of signature.
%
This choice of $p_I$ recovers the standard symmetry groups employed in particle physics and continuum mechanics.
...
The core insight driving a \textsc{SymmetryGAN} is deceptively simple.
%
...
This principle, when combined with a framework of inertial reference densities, yields a powerful methodology for symmetry discovery that is both theoretically sound and practically effective.
    ...
    \[
        L[g,d] = -\frac{1}{N}\sum_{i=1}^{N} \ln d(x_i) + \ln\bigl(1-d(g(x_i))\bigr)\;.
    \]
    ...
    Hence in equilibrium, the discriminator is maximally confounded with equal probability of classifying any sample as original or transformed, and the generator obeys precisely \cref{def:symmetry-of a pdf}.
    ...
    %
    This flexibility makes \textsc{SymmetryGAN}s applicable across diverse physical systems.
\begin{figure}
    \centering
...
\end{figure}
    \cref{fig:symmetrygan-schematic} shows the modified GAN architecture of  a \textsc{SymmetryGAN}.
    ...
    \begin{description}
        \item[Simultaneous discrimination:]
            ...
            %
            However, it requires the ability to sample from \(p_I\) which becomes problematic for improper priors like uniform distributions on \(\R^n\).
    ...
    %
    When the inertial distribution is uniform on \(\R^n\) the generator should be restricted to equiareal maps---transformations with unit Jacobian determinant, as shown in \cref{eq:equiareal-maps}.
    %
    The intersection of the equiareal group and affine group \(\operatorname{Aff}(n)\) is the affine special linear group \(\ASL(n)\) and its extensions.
...
\end{gather}
Hence every $g\in\operatorname{Aff}(2)$ admits the six parameter representation
\[
...
The Iwasawa parametrisation very well suited to gradient based optimisation over the space of affine symmetries.
...
Several complementary checks can be employed to determine whether the discovered transformation constitutes a true symmetry.
\begin{description}
    \item[Visual inspection:] Examination of low dimensional slices of the original and transformed data provides a preliminary, coarse grained, qualitative assessment of whether the transformation plausibly preserves the structure of the data.
    \item[Loss value:] For true symmetries, the binary cross entropy loss function converges to \( 2 \ln 2 \). Measuring the loss value for a discovered transformation therefore offers a quantitative criterion for symmetry verification.
    \item[Distribution matching:] Statistical divergences between \( X \) and \( g(X) \), such as the Kullback--Leibler divergence, quantify whether the two datasets share the same probability density.
    \item[Invariant verification:] Quantities known to be invariant under the symmetry, such as statistical moments, should be identical for both distributions.
\end{description}
    ...
    \subsection{Other symmetry discovery methods.}
        ...
        The Cramer--von Mises~\cite{von_mises_wahrscheinlichkeit_1928, cramer_composition_1928} and Anderson--Darling tests~\cite{anderson_asymptotic_1952} exemplify this approach to examine whether transformed data follows the same distribution as the original.
        %
        ...
        %
        Maximum Mean Discrepancy approaches~\cite{tolstikhin_minimax_2016} and other kernel methods can test for the existence of symmetry but struggle with continuous groups.
        ...
        %
        Latent LieGAN (LaLiGAN) enables the discovery nonlinear symmetries via latent space linearisation~\cite{yang_latent_2024}.
        %
        ...
        A \textsc{SymmetryGAN} both draws on and innovates on several important trends in modern machine learning for HEP.
        %
        ...
        %
        \textsc{SymmetryGAN}s also exemplify the trend towards interpretability by producing human understandable symmetry parameters rather than opaque features.
        %
        The method is however limited in its restriction to parametrisable symmetry classes.
        %
        It also exclusively enables symmetry discovery, i.e. the task of identifying elements of the symmetry group of a dataset.
        %
        ...
        The field continues to evolve rapidly, with \textsc{SymmetryGAN}s representing a significant step toward automated, theoretically grounded symmetry discovery.
        %
...
\sectionminitoc
        The theoretical machinery of a \textsc{SymmetryGAN}, with its rigorous statistical framework and inertial reference densities, proves its worth through empirical validation.
        %
        ...
        %
        This section details the application of this method on carefully controlled Gaussian experiments to complex collider data, demonstrating how \textsc{SymmetryGAN}s bridge the gap between mathematical elegance and practical discovery.
 ...
 \begin{figure}
     \centering
     \includegraphics[width=0.75\linewidth]{figures/chapter-09/Z2analytic.pdf}
     \caption[\textsc{SymmetryGAN}: Analytic loss landscape for $\mathcal{N}((0.5,1))$]{Analytic loss landscape for the one dimensional normal distribution $\mathcal{N}((0.5,1))$ under affine transformations $g(x) = b + cx$. The landscape exhibits two distinct maxima (indicated by stars) corresponding to the identity transformation $(b,c) = (0,1)$ and reflection $(b,c) = (1,-1)$, separated by a deep valley at $c = 0$.
     %
...
\begin{figure}
    ...
    %
    Random initialisation with $(b_i, c_i) \sim \mathcal{U}([-5, 5]^2)$ leads to convergence at two distinct fixed points. \textbf{(a)} Distribution of final parameters $(b_f, c_f)$ reveals two clusters at $(0,1)$ and $(1,-1)$, corresponding to the identity and reflection symmetries respectively. \textbf{(b)} Deterministic routing based on initial slope: $c_i > 0$ converges to identity $(c_f = 1)$ while $c_i < 0$ converges to reflection $(c_f = -1)$, with a sharp phase transition at $c_i = 0$. \textbf{(c)} Final intercept $b_f$ shows no correlation with initial value $b_i$, demonstrating that the loss landscape has no barrier in intercept space.}
    \label{fig:Z2numeric}
\end{figure}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.31\textwidth}
        ...
        \label{fig:MSE_iii}
    \end{subfigure}
    \caption[Controlled discretization of continuous $SO(2)$ symmetry into cyclic subgroups $\mathbb{Z}_q$ via constraint enforcement.]{Controlled symmetry breaking in the isotropic Gaussian $\mathcal{N}(\mathbf{0}, \mathbbm{1})$ through cyclic constraint enforcement. The loss function is augmented with a mean squared error term $\alpha \|g^q(\mathbf{x}) - \mathbf{x}\|^2$ that enforces $q-$fold cyclic symmetry. The continuous $SO(2)$ symmetry observed in \cref{fig:SO2-i} is discretised into cyclic subgroups: \textbf{(a)} $\mathbb{Z}_2$ symmetry with $q=2$, showing two antipodal maxima at $(c,s) = (\pm 1, 0)$; \textbf{(b)} $\mathbb{Z}_3$ symmetry with $q=3$, displaying three maxima at the cube roots of unity; \textbf{(c)} $\mathbb{Z}_7$ symmetry with $q=7$, exhibiting seven equally spaced maxima on the unit circle. The discovered symmetries (scatter points) precisely align with the $q^{\text{th}}$ roots of unity, validating the theoretical prediction that the cyclic constraint induces symmetry discovery of discrete components. Darker regions indicate higher loss values, stars indicate analytic maxima.}
    \label{fig:MSE}
\end{figure}               

                The two dimensional Gaussian example also allows us to test the approach described in \cref{eq:cyclic-loss}.
                %
                \cref{fig:MSE} shows the discovered symmetries for the \(N_{1, 1}\) distribution when the loss function is augmented with the constraint \cref{eq:cyclic-loss}, for \(q = \numlist{2;3;7}\), with \(\alpha = 0.1\).
                %
                ...
                %
                When the generator is allowed to search for general linear transformations inside \(\operatorname{Aff}(2)\), it discovers the full symmetry group that comprises the aforementioned Klein \(4-\)group as well as transformations involving the dilatation matrix \(D(r)\) described in \cref{thm:iwasawa_affine}.

                \cref{fig:AGL2symm} shows the discovered symmetries for the \(N_{1, 1}\) distribution and \cref{fig:AGL2asymm} shows the discovered symmetries for \(N_{1, 2}\) when the generator is parametrised to discover general affine transformations in \(\operatorname{Aff}(2).\)
\begin{figure}
    \centering
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
    ...
    \end{subfigure}
    \caption[Orthogonal slices of $\operatorname{Aff}(2)$ symmetries for isotropic Gaussian.]{Symmetry discovery in the full affine group $\operatorname{Aff}(2)$ for the isotropic Gaussian $\mathcal{N}(\mathbf{0}, \mathbbm{1})$, decomposed via Iwasawa parametrization $g = \sqrt{|d|} I^{\frac{1-\text{sgn}(d)}{2}} \cdot R(\theta) \cdot D(r) \cdot S(u) + \mathbf{v}$. Three orthogonal slices through the 6 dimensional loss landscape reveal the symmetry structure: \textbf{(a)} Determinant-rotation space $(d, \theta)$ shows continuous maxima along $d = \pm 1$ (vertical red lines) for all rotation angles. \textbf{(b)} Dilation-shear space $(r, u)$ exhibits a unique maximum at $(r, u) = (1, 0)$ (red star), indicating that only uniform scaling preserves the isotropic structure. \textbf{(c)} Translation space $(v_1, v_2)$ peaks at the origin (red star), verifying that the centred Gaussian admits no translational symmetries. Empirically discovered transformations (yellow points) cluster precisely at the theoretical optima, validating the analytic predictions.}
    \label{fig:AGL2symm}
\end{figure}
\begin{figure}
    \centering
    ...
    \end{subfigure}
    \caption[Discrete symmetry structure of anisotropic Gaussian in $\operatorname{Aff}(2)$ discovered by a \textsc{SymmetryGAN}.]{Symmetry discovery in $\operatorname{Aff}(2)$ for the anisotropic Gaussian $\mathcal{N}(\mathbf{0}, \text{diag}(1,2))$, revealing discrete symmetry breaking compared to the isotropic case (\cref{fig:AGL2symm}). The anisotropic covariance structure constrains the symmetry group to a supergroup of the Klein \(4-\)group $V_4$: \textbf{(a)} Determinant--shear space $(d, u)$ exhibits two isolated maxima (red stars) at $(d, u) \in \{(1, 0), (-1, 0)\}$, corresponding to orientation preserving and orientation reversing transformations with no shear. \textbf{(b)} Dilation--rotation space $(r, \theta)$ shows four discrete maxima (red stars) at $(r, \theta) \in \{(1, 0), (1, \pi), (\sqrt{2}, \pi/2), (\sqrt{2}, 3\pi/2)\}$, representing identity, $\pi$-rotation, and the two coordinate involutions scaled by $\sqrt{2}$ to account for the eigenvalue ratio. \textbf{(c)} Translation space $(v_1, v_2)$ remains peaked at the origin (red star), confirming no translational symmetries. The discovered transformations (yellow points) cluster at these discrete locations, demonstrating how anisotropy breaks continuous symmetries into a discrete group.}
    \label{fig:AGL2asymm}
\end{figure}
            \subsubsection{Gaussian Mixture Models}
                The power of a \textsc{SymmetryGAN} becomes more apparent when applied to Gaussian mixture models with complex symmetry groups.
                %
                ...
                \[
                    B(x) = \frac{1}{2} \mathcal{N}(-1, 1) + \frac{1}{2} \mathcal{N}(1, 1)
                \]
...
\begin{figure}
    ...
    \end{subfigure}
    \caption[\textsc{SymmetryGAN}'s predictions for a bimodal Gaussian mixture with $\mathbb{Z}_2$ reflection symmetry and corresponding loss landscape validation.]{Symmetry discovery for a one dimensional bimodal Gaussian mixture $p(x) = \frac{1}{2}\mathcal{N}(-1, 1) + \frac{1}{2}\mathcal{N}(1, 1)$ possessing $\mathbb{Z}_2$ symmetry. \textbf{(a)} Probability density function showing two symmetric modes at $x = \pm 1$ with equal weights and variances, creating a distribution invariant under reflection about the origin. \textbf{(b)} Analytic loss landscape in the $(b, c)$ parameter space for linear transformations $g(x) = b + cx$, with empirically discovered symmetries overlaid (blue points). The landscape exhibits two distinct maxima at $(b, c) = (0, 1)$ and $(0, -1)$, corresponding to the identity and reflection transformations respectively. The discovered symmetries cluster precisely at these theoretical optima, confirming that the \textsc{SymmetryGAN} correctly identifies the cyclic group $\mathbb{Z}_2 = \{x \mapsto \pm x\}$ as the complete symmetry group of the distribution.}
...
\begin{figure}
    ...
    \caption[Discovery of dihedral symmetries $D_8$ and $D_4$ in octagonal and square lattice Gaussian mixtures.]{Symmetry discovery for two dimensional Gaussian mixture models with non-commutative discrete symmetry groups. \textbf{Top row:} Octagonal distribution $p(\mathbf{x}) = \frac{1}{8}\sum_{k=0}^{7} \mathcal{N}(\mathbf{\mu}_k, 0.1\mathbbm{1})$ where $\mathbf{\mu}_k = (\cos\frac{2\pi k}{8}, \sin\frac{2\pi k}{8})$, possessing dihedral symmetry $D_8$. \textbf{(a)} Probability density showing eight equally-spaced modes on the unit circle. \textbf{(b)} Discovered rotations cluster at multiples of $\frac{\pi}{4}$, corresponding to the eight rotational symmetries. \textbf{(c)} Discovered reflections appear at eight angles, with antipodal points representing the same reflection axis. \textbf{Bottom row:} Square lattice distribution $p(\mathbf{x}) = \frac{1}{25}\sum_{i,j=-2}^{2} \mathcal{N}((i,j), 0.1\mathbbm{1})$ with dihedral symmetry $D_4$. \textbf{(d)} Probability density showing a $5 \times 5$ grid of Gaussian modes. \textbf{(e)} Discovered rotations concentrate at $0, \frac{\pi}{2}, \pi, \frac{3\pi}{2}$, identifying the four rotational symmetries. \textbf{(f)} Discovered reflections cluster at four angles corresponding to horizontal, vertical, and diagonal reflection axes. The precise alignment of discovered symmetries with theoretical predictions demonstrates \textsc{SymmetryGAN}'s ability to identify complex non-commutative groups.}
    \label{fig:otherdistributions-2D}
\end{figure}
                The second is a two dimensional octagonal distribution,
                \[
                    O(x) = \frac{1}{8}\sum_{i=1}^{8} \mathcal{N}(\cos\frac{2\pi i}{8}, 0.1)\times \mathcal{N}(\sin\frac{2\pi i}{8}, 0.1)
                \]
                which has the dihedral group of order 16, \(D_8\), as its symmetry group, a non-commutative discrete group containing both rotations and reflections.
                %
                The third is a \(5\times 5\) square distribution,
                \[
                    S(x) = \frac{1}{25}\sum_{i=0}^{4}\sum_{j=0}^{4} \mathcal{N}(i-2, 0.1)\times \mathcal{N}(j-2, 0.1)
                \]
                which has the dihedral group of order 8, \(D_4\), as its symmetry group, also a non-commutative discrete group containing both rotations and reflections.
                %
...
\end{figure}
           ...
           \textsc{SymmetryGAN}s produced similar results on both datasets.
            ...
            %
            i.e., the generator is restricted to the block diagonal form
            \[
...
]{Visual validation of discovered symmetry in dijet events through transverse momentum distributions.\textbf{ (a)} Original LHC Olympics showing the two leading jets' momenta $(p_x, p_y)$ in the transverse plane. The circular structure reflects the detector's azimuthal symmetry. \textbf{(b)} Same events transformed by a discovered generator from \textsc{SymmetryGAN}, showing identical statistical properties. The preservation of the ring structure and angular distribution confirms that the discovered transformation represents a genuine symmetry of the dijet system. Each point represents one event.}
...
\\
    ...
    {\footnotesize \textbf{(a)} and\textbf{ (b)} Azimuthal angle distributions after applying a random $SO(4)$ rotation to dijet events, demonstrating violation of symmetry. Transformed angles $\tilde{\phi}_1, \tilde{\phi}_2$ exhibit clear non-uniformity. $D_{KL}(p\|\mathcal{U}) \approx 0.15$ quantifies this deviation from uniformity, confirming that arbitrary $SO(4)$ rotations do not preserve the dijet distribution.
    \textbf{(c)} and \textbf{(d)} Azimuthal angle distributions after applying a discovered symmetry. Transformed azimuthal angles $\tilde{\phi}_1, \tilde{\phi}_2$ exhibit uniformity within statistical fluctuations. $D_{KL}(p\|\mathcal{U}) < 10^{-2}$ confirms agreement with the uniform distribution.}
...
\end{figure}
            ...
            Visual inspection can, however, provide a preliminary qualitative plausibility check.
            %
            ...
            \]
            %
            should remain uniformly distributed only for true symmetries, and not for any other transformation. This is indeed the case as is shown in \cref{fig:KL_rand_and_sym}.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        \label{fig:KLdiv_ii}
    \end{subfigure}
    \caption[Kullback-Leibler divergence analysis showing the transformations identified by the \textsc{SymmetryGAN} approach statistical noise floor.]{Quantitative validation of discovered symmetries via Kullback--Leibler divergence analysis. Histograms show $D_{KL}(\phi \| \tilde{\phi})$ between original and transformed azimuthal angle distributions for \textbf{(a)} the leading jet and \textbf{(b)} the subleading jet. Two distinct populations emerge: discovered symmetries (red) with means of 0.0058 and 0.0090 respectively, approaching the irreducible statistical noise floor (green) at $D_{KL} \approx 10^{-3}$ arising from finite sample size. Random $SO(4)$ rotations (blue) exhibit significantly higher divergences with means of 0.37 and 0.34, confirming that they violate the symmetry. Note the logarithmic scale emphasises the low divergence region, and so the red and green clusers are closer than they may appear.}
    \label{fig:KL-symm}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{figures/chapter-09/LHCOLosses.pdf}
    \caption[Loss values computed by training \textit{post hoc} discriminators to validate the transformations discovered by the \textsc{SymmetryGAN}.]{Discriminator loss distributions validating discovered symmetries against theoretical optimum. The histogram shows \textit{ post hoc} discriminator losses when trained to distinguish original from transformed dijet events. Discovered symmetries (blue) achieve losses tightly clustered around $2\log 2 \approx 1.386$ (dashed line), the theoretical minimum for perfect symmetries where transformed and original distributions are indistinguishable.
    %
    Random $SO(4)$ rotations (black) yield significantly lower losses, indicating the discriminator easily identifies these as non-symmetries. The discovered transformations achieve losses within 1\% of the theoretical optimum, providing independent confirmation that the \textsc{SymmetryGAN} identifies genuine invariances of the dijet system without prior physics knowledge.}
...
\end{figure}            
            ...
            %
            While this is never exactly true, \cref{fig:KL-symm} shows the KL divergence for the transformations identified by the \textsc{SymmetryGAN} approach the irreducible statistical noise floor, while arbitrary transformations from \(SO(4)\) clearly do not.
            
            Finally, \textit{post hoc} discriminators trained on discovered transformations achieve losses within 1\% of the theoretical optimum \(2\ln 2\) confirming these represent genuine symmetries, illustrated in \cref{fig:LHCOLosses}.
            %
        ...
        \subsection{Interpreting discovered symmetries.}
            ...
            One such example is
            \[
                g_{\mathbb{Z}_2}\mqty[p_{1x}\\ p_{1y}\\ p_{2x}\\ p_{2y}] = \mqty[-p_{2x} \\ -p_{2y} \\ -p_{1x} \\ -p_{1y}]\;,
            \]
            a transformation that simultaneously rotates by \(\pi\) and exchanges the two jets---a symmetry that follows from the identical nature of QCD jets but may not be obvious without a systematic search.        
            %
            ...
            The next challenge in this space is how to move from discovering individual symmetry transformations to understanding the complete symmetry group structure?
...
\section{Towards symmetry inference.}
            ...
            \begin{description}
                \item[Discrete subgroup analysis:] By modifying the loss function to enforce specific algebraic relations, a \textsc{SymmetryGAN} can systematically probe for discrete subgroups.
                %
                A \textsc{SymmetryGAN} trained with the cyclic group constraint, \cref{eq:cyclic-loss}, successfully identifies \(Z_q\) subgroups within larger symmetry groups, as seen in \cref{fig:MSE}.
                %
                ...
                \item[{Group composition methods}:] The closure property of groups suggests an iterative approach. Given discovered symmetries \(\{g_i\}\), one can attempt to compute all possible compositions \(g_i \circ g_j\) to expand the known group elements.
                %
                ...
                \item[The Symmetry Discovery Map:] Perhaps the most promising direction involves learning the complete mapping from initialisation space to symmetry space:
                \[
                ...
                Preliminary attempts to learn \(\Omega\) have had limited success.
                %
...
\sectionminitoc
    ...
    This penalty vanishes for distributions respecting the symmetry while penalising asymmetric solutions.
    %   
    ...
    %
    For a distribution over \(N\) bins with a \(\mathbb{Z}_k\) cyclic symmetry, the degrees of freedom reduce from \(N\) to \(\nicefrac Nk\), improving the effective statistics by a factor of \(k\).
    %
    ...
    \subsection{Symmetry preserving neural network architectures.}
        ...
        \[
            f * \psi = \int_H f(h)\psi(g^{-1}h) \dd h\;,
        \]
        where \(*\) denotes group convolution and \(h\in H\) is the stabiliser subgroup.
    ...
    \subsection{Reducing dimensionality through symmetry identification.}
        ...
        \begin{definition}[Group invariant function]
...
\label{thm:symmetry-adapted-cramer}
...
\label{sec:improved-measurement-prediction}
    ...
    \subsection{Data augmentation using discovered symmetries.}
        ...
        \begin{description}
            \item[Faster convergence:] The effective sample size increase should translate directly to fewer training epochs.
            \item[Better generalisation:] The network `sees' the full symmetry orbit during training, preventing overfitting to specific configurations.
            \item[Automatic equivariance:] Even non-equivariant architectures should learn equivariant information from augmented data.
        \end{description}
...
\chapterminitoc
    This chapter began with the seemingly simple question of what it means for a dataset to possess symmetry, only to wade through layers of mathematical formalism involving Jacobian factors, inertial reference densities, and the interplay between statistical and physical invariances.
    %
    ...
    \subsection{Beyond linear symmetries.}
        ...
        \]
         ...
         The H\'enon map is but one of a vast and relatively uncharted space of the full (nonlinear) \emph{equiareal group}.
        %
        ...
        \subsubsection{HEP applications.}
        A few examples of particle physics applications of non-linear symmetries are
        \begin{description}
            \item [Jet substructure:] The Lund plane reveals approximate scale invariant symmetries best expressed through logarithmic transformations~\cite{Dreyer:2018nbf}.
            \item [Heavy flavor physics:] Dalitz plot analyses exhibit non-linear symmetries from resonance dynamics~\cite{Asner:2003gh}.
            \item [Multiparticle correlations:] Collective flow in heavy ion collisions follows non-linear hydrodynamic symmetries~\cite{Yan:2015lwn}.
        \end{description}
    ...
    \subsection{Approximate symmetries and symmetry breaking.}
        ...
        %
        A transformation \(g_\epsilon\) depending on breaking parameter \(\epsilon\) is an approximate symmetry if
        \[
            p(g_\epsilon(x)) |g'_\epsilon(x)| = p(x) + \epsilon \Delta p(x) + O(\epsilon^2)\;.
        \]
        
        The firstorder breaking term \(\Delta p(x)\) encodes how symmetry violation depends on phase space location, crucial information for precision measurements.
        ...
        %
        Instead of binary classification (symmetric or not symmetric), one could attempt to quantify the degree of symmetry through continuous measures, such as 
        \[
    ...
    \subsection{A unified framework.}
        ...
        %
        Every symmetry provides a consistency check, where violations of expected symmetries signal systematic uncertainties, methodological gaps, or perhaps, new physics.
        %
        The discovered symmetries are also themselves physics outputs.
        %
        An understanding of which symmetries hold and which ones are broken in any given situation are not only of applied interest, but also of fundamental theoretical interest in the study of the laws of physics.
        ...
        %
        The marriage of machine learning's ability to detect and learn patterns with physics' principled reasoning promises new paradigms of analysis, where every symmetry discovered is both a practical tool and a theoretical insight.
