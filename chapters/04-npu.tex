\chapter{\textsc{Neural Posterior Unfolding}}
\label{chap:npu}
\chapterminitoc
\section{Motivation for improved binned unfolding}
\sectionminitoc

\begin{definition}[Binned unfolding]
...
\subsection{Regularisation limitations.}
    To address the ill posedness detailed in \cref{sec:ill-posed}, traditional methods employ various forms of regularisation, examples of which are provided in \cref{sec:binned-methods}.
    %
    ...
    %
    Traditional unfolding methods typically do not provide a natural framework for propagating uncertainties and can significantly underestimate the true uncertainty in degenerate regions.
    %
    ...
    \begin{enumerate}
        ...
        \item Reduce dependence on arbitrary regularisation parameters, and
    ...
    %
    Additionally, the regularisation in NPU emerges implicitly from the neural network architecture and training protocol, reducing the need for manual tuning of regularisation parameters.

    The following sections introduce normalising flows and neural posterior estimation as the foundation for the NPU method, demonstrating how these modern machine learning techniques address the fundamental challenges of binned unfolding.
...
\subsection{A Bayesian perspective on inverse problems.}
    ...
    \]
    where \(p(x\mid z) =  r(x\mid z)\) is the detector response, $p(z)$ is the prior for particle level quantities, and $p(x)$ is the evidence.
    
    Traditional methods typically focus on finding a point estimate that maximises this posterior, but a full Bayesian treatment would characterise the entire posterior distribution to properly account for uncertainties and degeneracies.

    Normalising flows offer several key advantages for modelling the posterior distribution in inverse problems.
    \begin{description}
        \item[Complex distribution modelling:] Unlike standard parametric approaches, normalising flows can represent multimodal, asymmetric posteriors that often arise in inverse problems with degeneracies where multiple particle level configurations can lead to similar detector level observations.
        \item[Amortised inference:] Once trained, normalising flows enable rapid posterior sampling without requiring new optimisation runs for each new observation.
        %
        ...
        \item[End--to--end differentiability:] The entire posterior estimation process can be optimised end to end, enabling gradient based learning approaches that are more efficient than traditional MCMC methods for high dimensional unfolding.
        \item[Implicit regularisation:] Neural network architectures naturally introduce an inductive bias that serves as implicit regularisation, potentially reducing overfitting to statistical fluctuations compared to direct matrix inversion approaches without requiring explicit regularisation parameters.
    \end{description}
\subsection{Conditional normalising flows for inverse problems.}
    In the context of unfolding, one is specifically interested in modelling the conditional distribution $p(z\mid x)$, the particle level distribution given detector level observations.
    %
    Conditional normalising flows, described in \cref{subpar:conditional-nfs} are an architecture almost perfectly designed for this task.
    %
    ...
    \begin{enumerate}
        ...
        \item The computational cost of generating samples does not increase with the dimensionality of the parameter space.
...
\subsection{Statistical foundation.}
    ...
    %
    The fundamental insight is to directly estimate the conditional posterior distribution \(p(z\mid x)\) through \(\P(\vb*\nu\mid \vb*\mu)\) the probability that the true particle level histogram counts are $\vb*\nu$ given the measured detector level histogram counts are $\vb*\mu$.
...
\subsection{Machine learning architecture.}
    ...
    %
    an invertible transformation, a learnable, invertible function $f_\theta(u\mid\vb*\mu)$ that transforms samples from the base distribution into samples from the approximate posterior, conditioned on the measured data $\vb*\mu$, and
    %
    a density transformation, encoding the the change of variables formula,
    \[
        \P_\theta(\vb*\nu\mid\vb*\mu) = \P_u(f_\theta^{-1}(\vb*\nu\mid\vb*\mu))\cdot \qty|\det\qty{ \grad_{\vb*\nu}f_\theta^{-1}(\vb*\nu\mid\vb*\mu)}|\;.
    \]
    
    The neural network parameters $\theta$ are optimised to maximise the likelihood of the true posterior, using pairs of simulated $(\nu_i, \mu_j)$ examples.
    %
    ...
    The implementation uses a normalising flow based on the MADE (Masked Autoencoder for Distribution Estimation) architecture~\cite{papamakarios_masked_2018}.
    %
    The model is composed of an invertible transformation network with three fully connected layers, each containing \numrange{50}{100} nodes with Swish activation functions~\cite{Ramachandran2017SearchingFunctions}.
    %
    Conditional inputs incorporated through an auxiliary fully connected layer.
    %
    The network is then trained for \numrange{1000}{1500} epochs using the \textsc{Adam} optimiser with a learning rate of \num{d-4} and a batch size of \num{d4}.
    %
...
\subsection{Addressing degeneracy with NPU.}
    ...
    %
    In traditional methods like IBU, the unfolded result becomes entirely dependent on the prior and the method incorrectly reports zero uncertainty on the ratio \(\nicefrac{\nu_\alpha}{\nu_\alpha+\nu_\beta}.\)
...
\subsection{Implicit regularisation in NPU.}
    ...
    %
    Several mechanisms contribute to this implicit regularisation.
    \begin{description}
        \item [Network capacity:] The finite capacity of the neural network limits the complexity of the posterior approximation, preventing overfitting to statistical noise in a manner similar to how traditional regularisation constrains solution complexity.
        \item [Smooth transformations:] Normalising flows use smooth transformation functions, which naturally bias the solution toward smooth posterior distributions rather than ones with sharp, unphysical features.
        \item [Amortised inference:] By learning a conditional distribution that applies across many possible measurements, NPU averages over many training examples, reducing sensitivity to outliers or statistical fluctuations in any single measurement.
        \item [Architectural inductive bias:] The specific architecture of the normalising flow introduces an inductive bias.
        %
        ...
        \item [Optimisation dynamics:] The stochastic gradient descent training process itself provides regularisation through early stopping based on validation performance, preventing overfitting to the training data.
    \end{description}
    
    This implicit regularisation offers some advantages over the explicit regularisation used in traditional methods.
    %
    It adapts automatically to the complexity of the problem rather than requiring manual tuning of regularisation parameters.
    %
    ...
    The next section will demonstrate the application of NPU to concrete examples, including both controlled Gaussian distributions and realistic jet substructure observables, to illustrate its performance in practice.
...
\sectionminitoc
    ...
    %
     Consider first, a simple two--bin degenerate case to illustrate NPU's advantages in handling unconstrained regions of phase space, before moving to a Gaussian example to systematically assess statistical performance, and finally studying the method's efficacy on realistic jet substructure simulation from the LHC.
     
    \subsection{2 bin degenerate response example.}
    
        The first example illustrates NPU's behaviour by honing in degenerate scenarios where traditional methods struggle.
        %
         Construct a two--bin setup where $\vb*{\mu}, \vb*\nu \in \mathbb{R}^2$ with a response matrix $\mathbf{R} \in \mathbb{R}^{2 \times 2}$.
        %
        Fix the Truth values at $\nu_0 = \nu_1 = \num{5e4}$ and parametrise the response matrix using a correlation coefficient $\rho$ and diagonal elements $\sigma_0 = \sigma_1 \equiv \sigma = 0.8$.

When $\rho = 0$, the response matrix is well conditioned with a diagonal structure, meaning that each detector bin primarily receives contributions from a single truth bin.
%
...
However, as \(\rho\to1,\) the response matrix becomes degenerate, i.e., $R_{i\alpha} = R_{i\beta}$, because the detector loses its ability to differentiate between the two Truth bins.
%
This degeneracy means that multiple Truth configurations can produce identical Data observations, making the unfolding problem underconstrained.
...
\begin{figure}
    ...
    \caption[NPU: 2 bin example]{Comparison between NPU and IBU using a two bin example.
    %
    ...
    %
    \textbf{(a)} Non degenerate, diagonal response matrix $(\rho = 0)$: 
    ...
    The NPU maximum likelihood estimate closely aligns with the true distribution, and the NPU posterior is compatible with the statistical uncertainty evaluated by IBU using bootstraps.Figure created by Jingjing Pan~\cite{acosta2024npu}.
    }
    \label{fig:2bin}
\end{figure}
...
a Generation dataset with \num{d6} events for training, and a Truth dataset with \num{d5} events for validation.

To simulate detector effects, apply Gaussian smearing with parameter $\epsilon = 0.5$ to both particle level datasets.
%
This produces corresponding detector level distributions, Simulation, from Generation, and Data, from Truth, both with the same mean but with increased width $\sigma_\text{detector} = \sqrt{1^2 + 0.5^2} \approx 1.12$.
...
\begin{figure}
    ...
    \caption[NPU: Gaussian example setup]{Gaussian unfolding example setup. 
    ...
    %
    The matrix shows how probability mass spreads from each particle level bin (horizontal axis) to detector level bins (vertical axis).Figure created by Jingjing Pan~\cite{acosta2024npu}.
    }
    \label{fig:gaus_init}
\end{figure}
\subsubsection{Results and comparison.}
    ...
    %
    Beyond point estimates, NPU provides a complete characterisation of the uncertainty through its full posterior distribution, including the bin--to--bin correlations displayed in the corner plot in \cref{fig:gaus:b}.
    %
...
\caption[NPU: Gaussian unfolding results and posterior analysis]{Gaussian unfolding results comparison.
...
%
The green for FBU is averaged over many draws. Figure created by Jingjing Pan~\cite{acosta2024npu}.
}
\label{fig:gaus}
\end{figure}
    ...
    Recall from \cref{subsubsec:pull-distributions,thm:pull-normality} that for properly calibrated uncertainty estimates, these pulls should follow a standard normal distribution with zero mean and unit variance.
    ...
    %
    One can further test the robustness of this calibration by varying the smearing parameter across $\epsilon \in [0.3, 0.6]$, finding that both methods maintain proper coverage throughout this range.
    %
    \cref{fig:pulls:b} demonstrates the calibration as function of smearing parameters for various methods.
...
The mean and standard deviation remain consistent with the ideal values ($\mu = 0$, $\sigma = 1$) across the range, confirming robust uncertainty quantification for both methods. Figure created by Jingjing Pan~\cite{acosta2024npu}.
}
\label{fig:pulls}
\end{figure}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter-04/npu_width_logy.pdf}
        \caption{Jet width $\omega$}
        \label{fig:phys_width}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter-04/npu_zgs.pdf}
        \caption{Groomed momentum fraction $z_g$}
        \label{fig:phys_zg}
    \end{subfigure}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter-04/npu_tau2s_logy.pdf}
        \caption{$N$-subjettiness ratio $\tau_{21}^{\beta=1}$}
        \label{fig:phys_tau21}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter-04/npu_mult_logy.pdf}
        \caption{Constituent multiplicity $M$}
        \label{fig:phys_mult}
    \end{subfigure}
    
    \caption[Jet substructure unfolding results using NPU, FBU, and IBU]{
        (Continued from previous page)
        %
        Unfolding results for jet substructure observables comparing Neural Posterior Unfolding (NPU), 
        Forward Bayesian Unfolding (FBU), and Iterative Bayesian Unfolding (IBU) methods. 
        The response matrix is constructed using \textsc{Pythia} 8.243 (Tune 21) MC distributions, 
        while \textsc{Herwig} 7.1.5 provides both the Data to unfolding and Truth
        for validation. Truth distributions are shown as blue shaded regions, with unfolded 
        results from NPU (maximum likelihood estimate as orange points), FBU (green points), 
        and IBU (red points). Error bars represent posterior standard deviations from the 
        respective methods. NPU successfully recovers the particle-level truth distributions 
        across all observables. Figure created by Jingjing Pan~\cite{acosta2024npu}.%
    }
    \label{fig:substructure}
\end{figure}

    This experiment demonstrates NPU's application to a realistic high energy physics scenario, focusing on jet substructure measurements at the Large Hadron Collider (LHC).
    ...
    \subsubsection{Dataset and Observables}
        ...
        \begin{description}
            \item[\textsc{Herwig 7.1.5}~\cite{Bellm2017HerwigNote}] serves as the data and truth distributions, and
        ...
        This analysis focus on four key jet substructure observables that are widely used in LHC analyses.
        \begin{description}
            ...
            \item [Groomed momentum fraction ($z_g$):] the momentum shared between subjets after soft drop grooming~\cite{Larkoski2014SoftDrop, Dasgupta2013TowardsSubstructure}
        \end{description}
    ...
    \subsubsection{Results}
        ...
        %
        Second, the method exhibits robustness against simulation modelling differences: the response matrix is constructed using \textsc{Pythia} events, while the ``nature'' distribution NPU was expected to unfold was derived from \textsc{Herwig}, representing different underlying physics models and parton shower algorithms.
        %
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter-04/corner_both_width.pdf}
        \caption{Jet width $\omega$}
        \label{fig:corner_width}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter-04/corner_both_zgs.pdf}
        \caption{Groomed momentum fraction $z_g$}
        \label{fig:corner_zg}
    \end{subfigure}
    \caption[]{(continued on next page)}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter-04/corner_both_mult.pdf}
        \caption{Constituent multiplicity $M$}
        \label{fig:corner_mult}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter-04/corner_both_tau2s.pdf}
        \caption{$N$-subjettiness ratio $\tau_{21}^{\beta=1}$}
        \label{fig:corner_tau21}
    \end{subfigure}
    
    \caption[Corner plots for jet substructure observables]{%
        (Continued from previous page)
        %
        Corner plots showing posterior correlations for jet substructure observables. 
        Each panel displays pairwise correlations between bins for one observable, 
        comparing Neural Posterior Unfolding (NPU, orange contours) and Forward Bayesian 
        Unfolding (FBU, green contours, averaged over Markov Chain Monte Carlo samples) 
        posteriors, with truth values marked in blue. The plots reveal complex bin-to-bin 
        correlation structures that are typically unavailable from traditional unfolding 
        methods. Strong agreement between NPU and FBU posteriors confirms the reliability 
        of both approaches, while the encompassing of truth values validates proper 
        uncertainty quantification. These correlations provide crucial information for 
        propagating uncertainties in downstream physics analyses. Figure created by 
        Jingjing Pan~\cite{acosta2024npu}.%
    }
...
\subsection{Summary of numerical results.}
    ...
    %
    NPU efficiently processes multiple datasets through amortised inference, and accurately recovers truth distributions in both targeted degenerate toy examples and realistic high energy physics scenarios.
    %
    ...
    %
    By providing differentiable access to the posterior density, NPU enables gradient based optimisation for finding maximum likelihood estimates or other derived quantities.
...
\section{Beyond binning: the path forward.}
    ...
    %
    The idea that the choice of binning is subjective and can introduce biases in the unfolded results is oft expressed in statistical treatments of the unfolding problem~\cite{cowan_survey_2002, cowan_statistics_2021, CowanStatisticalRehovot, Cowan2011AsymptoticPhysics,Tang2017DataMethod, hocker_svd_1996, Gagunashvili2024DataOptimization, CranmerPracticalLHC, StatisticalPhysics, Bohm2025IntroductionPhysicists, gardi_statistics_2015, Adam-Bourdarios2015TheChallenge, cowan_topics_2010}.
    %
    ...
    %
    The restrictions the curse of dimensionality imposes on binned methods' applicability to multivariate problems has already been discussed in \cref{subsubsec:binned-methods}.
    ...
    %
    Although machine learning methods are typically a few orders of magnitude slower to train than traditional methods like IBU, modern ML techniques can leverage efficient data representations and parallelised computation for faster inference, reducing the gap particularly for high dimensional problems.
    ...
    %
    The transition from binned to unbinned unfolding represents not just a technical evolution but a paradigm shift in how the field approaches the measurement and interpretation of cross section measurements.