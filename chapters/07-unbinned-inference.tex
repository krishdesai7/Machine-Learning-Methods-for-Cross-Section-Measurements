...
\sectionminitoc
\begin{definition}[Event independence in HEP]
...
\end{definition}
    ...
    \subsection{Poisson point processes for event counts.}
        ...
        \[
            \P( N\mid\theta) = \frac{e^{-\mu(\theta)}\,\mu(\theta)^N}{N!}.\]
            
        This reflects the physical picture of interaction events occurring as Poisson point processes in time, with each event occurrence independent of the last~\cite{cowan_statistical_1998, Kuusela2015StatisticalQuantification}.
        %
    ...
    \subsection{Independent and identically distributed (i.i.d.) events.}
        ...
        %
        The identical distribution assumption further requires that the experimental conditions remain stable so that each interaction is sampled from the same probability density function $p(x\mid \theta)$.
        %
        ...
        %
        that the data are viewed as a Poisson sample of independent events from $p(x\mid\theta)$.
        %
    ...
    \subsection{Likelihood factorisation and conditional independence.}
        ...
        \] 
        which in turn leads to a factorised likelihood function.
        
        In the common case where $N$ itself is treated as Poisson distributed, the full extended likelihood is 
        \begin{equation}
            \mathcal{L}(\theta) \;=\; e^{-\mu(\theta)} \frac{\mu(\theta)^N}{N!}\;\prod_{i=1}^{N} p(x_i\mid \theta)\,,
        ...
        \begin{equation}
            \ln \mathcal{L}(\theta) = -\mu(\theta) + \sum_{i=1}^{N} \ln p(x_i\mid\theta)\,,
            \label{eq:loglik-sum}
        \end{equation}
        up to the constant term $\ln(N!)$.\footnote{In cases where $N$ is fixed and not of interest, the $-\mu + \frac{\mu^N}{N!}$ portion is often dropped, yielding the simplified unbinned likelihood $\prod_{i}p(x_i\mid\theta)$.}
...
\section{Violation of statistical independence.}
    ...
    By treating events as independent, analyses can take full advantage of powerful likelihood--based inference techniques and modern machine learning methods that operate on event--by--event data~\cite{GomezAmbrosio:2022mpm, mishra-sharma_smsharmaawesome-neural-sbi_2025, noauthor_simulation-based-inferencesimulation-based-inferencegithubio_2025}.
    %
    Unbinned machine learning approaches to parameter estimation, for instance, explicitly rely on the factorised likelihood in \cref{eq:loglik-sum} to avoid information loss from binning~\cite{GomezAmbrosio:2022mpm, Arratia:2021otl, du_unifying_2024}.
    %
    ...
    \subsection{Detector effects.}
        ...
        These effects often manifest as systematic uncertainties in the analysis.
        %
    ...
    \subsection{Pileup.}
        ...
        In nucleus--nucleus (heavy ion) experiments the main contaminant is not separate overlapping interactions but the extremely high multiplicity
        ...
        %
        Instead one is truly measuring conglomerate events with extra particles, and the probability of certain features (e.g. multiple soft jets, high detector occupancy) is higher than what a single interaction $p(x\,|\,\theta)$ would predict.
    ...
    \subsection{Unfolding and data processing.}
        ...
        %
        Thus, the set of unfolded events has an empirical global covariance structure because knowledge of one event's outcome gives information about others.
        
        If one were to na\"ively apply an unbinned likelihood analysis assuming those unfolded events are independent, the likelihood factorisation would be incorrect, and one might misestimate uncertainties or bias the fit.
        %
        This situation has been explicitly observed.
        %
        The work presented in this chapter~\cite{Desai:2025mpy} shows that treating unfolded events as independent can misestimate the confidence intervals, with asymptotic error formulae significantly misestimating the true uncertainty.
        ...
        For instance, if an unfolded spectrum is used to extract a physics parameter, data is provided in set bins alongside a covariance matrix $\Sigma$ such that a binned $\chi^2$ fit can be performed,
          ...
          \] 
        where $D$ is a vector of normalised bin counts of the unfolded data and $\P(\theta)$ the corresponding model prediction~\cite{D0:2020ujb, CMS:2024onh, Gribanov:2022gri, Canonero:2024kzk}.
        ...
        It would therefore be prudent to avoid analytic uncertainty formulae in such cases and rely on bootstraps to evaluate uncertainties.
        %
    ...
    \subsection{Global constraints.}
        ...
        Even when individual interactions are truly independent, certain \emph{global} aspects of an analysis can couple what were otherwise independent events.
        %
        ...
        %
        Reformulated in Bayesian terms, if one marginalises over an uncertain global parameter, the remaining distribution of events is no longer a simple product of independent probability density functions.
        ...
        %
        These are exotic cases but illustrate the broader point that nature can produce data that do not align with the one--event--at--a--time paradigm.
...
\section{Formalism.}
\label{sec:formalism}
    Let $\qty{\nu_j}_{j=1}^M$ be the unfolded, normalised particle level yield in bin $j$ and $\P_j(\theta)$ be the theory prediction or fit template for that bin given parameters $\theta$.
    %
    ...
    \begin{gather}
       ...
       \eval{\mathrm{Err}(\bar{X})}_{\rho=-\frac{1}{N-1}} = 0,\quad
       \eval{\mathrm{Err}(\bar{X})}_{\rho=0} = \frac{\sigma}{\sqrt{N}}, \quad \eval{\mathrm{Err}(\bar{X})}_{\rho = 1} = \sigma.
    \end{gather}
...
\begin{equation}\label{eq:equicorr-variance}
...
\end{proof}
    ...
    %
    Developing such a formalism\footnote{For example, an approach using copulas to model the joint PDF of all events, or incorporating the covariance matrix into a generalised likelihood.} is left to future research.
    %
...
\end{theorem}
        ...
        \subsection{Godambe information: the sandwich estimator)}
            ...
            %
            This approach rigorously generalises the notion of `effective degrees of freedom' due to correlations.
...
\end{theorem}
    ...
    \subsection{Numerical resampling approaches: bootstrapping and toy Monte Carlo.}
        ...
        \subsubsection{Non-parametric bootstrap resampling.}
            ...
            %
            By analysing all replicas identically, one can extract not just variances for each observable but the full covariance matrix via the sample covariance of the replica results~\cite{noauthor_evaluating_2021}.
            %
            ...
            %
            With sufficient computational resources available, one could resample whole event with replacement to build each replica dataset, thus preserving event correlations.
            ...
            \begin{definition}[Poisson bootstrap]
            ...
            The expected total count in any replica of a Poisson bootstrap is then also $N$ (since $\sum_{i=1}^N \text{Poisson}_i(1) \sim \text{Poisson}(N)$, which has expected value \(N\)), but the total count of any individual replica is no longer fixed at \(N\).
            %
        ...
        \subsubsection{Parametric bootstrap: Monte Carlo simulations.}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
    \algospacing
    \Require{Observed data $X_{1},\dots,X_{N}$, model $p(x\mid\theta)$, number of replicates $B$}
    \Ensure{Bootstrap estimates $\{\hat\theta^{(b)}\}_{b=1}^{B}$ or distribution of a statistic}
    
    \State $\hat\theta \leftarrow \displaystyle\arg\max_{\theta}\,\prod_{i=1}^{n}f(X_{i};\theta)$
    \For{$b\leftarrow 1$ \textbf{to} $B$}
...
\end{algorithm}
        ...
        %
        \cite{noauthor_evaluating_2021} suggests that on the order of $B=\num{1000}$ replicas is a reasonable compromise in many cases, though even $B=\num{10000}$ is used for certain jet cross section covariance evaluations.
    ...
    \subsubsection{Practical considerations and guidelines.}
        ...
        \paragraph{Number of replicas.}
            ...
            %
            It is good practice to check convergence of the uncertainty estimate: e.g. run \(B = \numlist{200;400;800}\) and monitor if the reported uncertainty or key figure of merit changes appreciably.
            %
            If it stabilises, $B$ is sufficient.
            
            The standard error on a bootstrap derived standard deviation is $\nicefrac{\sigma}{\sqrt{2(B-1)}},$ so increasing \(B\) yields diminishing returns.
        \paragraph{Computational cost and parallelisation.}
            ...
            %
            In a modern analysis framework, it is often feasible to distribute $B=\num{1000}$ jobs to a cluster and retrieve results in a few hours, where running them serially would be prohibitive.
            %
        ...
        \paragraph{Common pitfalls.}
            \begin{description}
                ...
                \item[Interpreting bootstrap outcomes:]
...
\sectionminitoc
    \subsection{Setup.}
    \label{subsec:setup}
        A Truth distribution is constructed from a Gaussian with mean $\mu_{T}=0.2$ and variance $\sigma^2_{T}=0.81$.
        %
        \num{d4} events are drawn from this distribution to serve as the dataset at the particle level.
        %
        In addition, a larger Monte Carlo (MC) sample of \num{e5} events is generated from a Gaussian with mean \(\mu_G = 0.0\) and variance \(\sigma^2_G = 1.0.\)
        %
        ...
        %
        Multiple smearing levels are examined, with $\sigma_{\text{det}}$ ranging from \(0\), no smearing, i.e. a perfect detector, up to \(0.75\), substantial resolution degradation.
        %
    ...
    \label{subsec:fully_binned_demo}
        ...
        This demonstration will highlight the importance of properly accounting for the covariance matrix of the unfolded result when making inferences on physics parameters.
         ...
         \num{500} independent pseudo--datasets are generated from the Truth/Data model, each with a Poisson fluctuation in total event count with a expectation value of \num{e4} events per dataset to simulate realistic statistical scatter.\footnote{The study as presented uses pseudo--experiments of a Poisson fluctuated size rather than a non-parametric Poisson bootstrap due to the ease of generating Gaussian examples, but essentially results were replicated using a non-parametric Poisson bootstrap proper, and the observations and analysis carry over essentially unchanged.}
        
        \subsubsection{Methodology.}
        \label{subsec:methodology}
            Bin all the datasets using a fixed histogram binning. For this example, 15 uniform bins are used spanning the range $[-3,4]$.\footnote{The bin range chosen is asymmetric around 0 because \(\mu_T = 0.2 > 0\).}
            %
            ...
            %
            \cref{fig:smearing-examples} shows the Simulation distribution at smearnings \(\sigma_{\text{det}}\in \qty{\numlist[list-final-separator = {, }]{0.00; 0.25; 0.50; 0.75}}\) and the corresponding response matrices.
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption[]{Particle level distributions comparing Truth (blue) with parameters $\mu_{T} = 0.2, \sigma^2_{T} = 0.81$ with Generation (orange) with parameters $\mu_{G} = 0.0, \sigma^2_{G} = 1.00$.
    %
    The histograms represent normalised frequency distributions and the dashed curves show the analytic density.}
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption[]{Detector level distributions after applying Gaussian smearing with resolution $\sigma_{\text{det}} = 1.0$. Data (blue) has parameters $\mu_D = 0.2$ and $\sigma^2_D = 1.81$, while Simulation (orange) has $\mu_S = 0.0$ and $\sigma^2_S = 2.00$.}
    \label{fig:detector-level}
  \end{subfigure}
  \caption[Detector and particle level distributions for the study on correlations during inference.]{Comparison of particle level and detector level distributions. \textbf{(a)} shows the particle level distributions where the Truth (blue histogram and dashed curve) differs from the Generation (orange histogram and dashed curve). \textbf{(b)} demonstrates the effect of finite detector resolution through Gaussian smearing with $\sigma_{\text{det}} = 1.0$, which convolves with the underlying distributions to produce broader observed distributions at the detector level.
  }
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/chapter-07/unbinned_cor-smearing_examples.pdf}
    \caption[]{\small Detector level distributions as a function of detector resolution $\sigma_{\text{det}}$.
    %
    Step histograms show the normalised density of Simulation after applying Gaussian smearing with $\sigma_{\text{det}} \in \{\numlist[list-final-separator = {, }]{0.00; 0.25; 0.50; 0.75}\}$. Dashed curves represent the corresponding analytic Gaussian with variance \(\sigma^2_{G} + \sigma^2_{\text{det}}\).
    }
    \label{fig:smearing-examples}
  \end{subfigure}\hfill\begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/chapter-07/unbinned_cor-response_matrix.pdf}
    \caption[]{\small Discretised response matrices $R(\vb*\mu_i\,|\,\vb*\nu_j)$ for detector resolutions \numlist{0.00; 0.25; 0.50; 0.75}. Each row is normalised so that $\sum_i R(x_i\mid z_j) = 1$ for all $j$. Observe the transition from a diagonal matrix (perfect resolution) to increasingly off diagonal structures as detector smearing increases.}
    \label{fig:response-matrices}
  \end{subfigure}
  \caption[Detector response effects for smearings \numlist{0.00; 0.25;0.50;0.75}]{\small Characterisation of detector response effects through progressive smearing of Generation. \textbf{(a)} illustrates how Generation is broadened by convolution with Gaussians of width $\sigma_{\text{det}}$. The variance adds in quadrature: $\sigma^2_S = \sigma^2_G + \sigma^2_{\text{det}}$.
  %
    \textbf{(b)} presents the corresponding response matrices, which encode the probability of bin migration due to detector effects.
  %
  For perfect resolution ($\sigma_{\text{det}} = 0$), the response matrix is diagonal, indicating no bin migration.
  %
  As detector resolution degrades, off diagonal elements become increasingly prominent, as events are reconstructed in neighbouring bins.
  }
    ...
    \resizebox{!}{0.5\textheight}{%
        \begin{minipage}{\textwidth}
            ...
            \begin{subfigure}[t]{0.48\textwidth}
                ...
                \caption[]{\small Asymptotic and bootstrap RMS uncertainty on the Gaussian mean $\mu$ as a function of $\sigma_{\text{det}}$.
                %
                The lower panel displays the ratio of asymptotics to the bootstrap RMS, demonstrating systematic overestimation of uncertainties when correlations are ignored.}
            ...
            \begin{subfigure}[t]{0.48\textwidth}
                ...
                \caption[]{\small Asymptotic and bootstrap RMS uncertainty on $\sigma^2$ as a function of $\sigma_{\text{det}}$.
                %
                The diagonal approximation increasingly overestimates the uncertainty as resolution degrades, indicating that neglecting correlations leads to measurable over-coverage.}
                \label{fig:var_error_plot_with_errorbars}
            \end{subfigure}
    \caption[Comparison of uncertainty estimates from asymptotics and bootstraps for binned unfolded distribution parameters as a function of detector resolution.]{\small Comparison of uncertainty estimates for binned unfolded distribution parameters as a function of resolution.  (a) and (b) show uncertainties for the mean $\mu$ and variance $\sigma^2$, respectively. The analysis compares (i) asymptotics using the full covariance matrix (green circles), (ii) asymptotics using the diagonal covariance approximation that neglects off-diagonal terms (pink squares), and (iii) empirical validation through 500 pseudo-experiments (green stars). The excellent agreement between the full covariance asymptotic errors and bootstrap derived uncertainties validates the analytical error propagation. The diagonal approximation consistently overestimates uncertainties, with the discrepancy growing with detector smearing. Thus, proper treatment of the full covariance is essential for when extracting distribution parameters.}
...
\begin{figure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Mean best-fit values of $\mu$ as a function of $\sigma_{\text{det}}$.
        %
        The lower panel shows the ratio to truth, demonstrating unbiased parameter extraction within statistical uncertainties across all resolutions.}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        ...
        \caption{Mean best-fit values of $\sigma^2$ as a function of $\sigma_{\text{det}}$. The slight systematic trend toward misestimation at large $\sigma_{\text{det}}$ may arise from regularisation bias in the unfolding or binning effects.}
        \label{fig:var_mean_values_with_errorbars}
    \end{subfigure}
    \caption[Validation of unbiased parameter extraction from unfolded distributions across varying detector resolutions.]{Parameter extraction from unfolded distributions across varying detector resolutions. \textbf{(a)} and \textbf{(b)} display the mean best-fit values for the mean $\mu$ and variance $\sigma^2$ obtained by fitting unfolded distributions. The analysis demonstrates that both the full covariance (green circles) and diagonal approximation (pink squares) yield unbiased central values, recovering the true parameters (red dashed lines) within statistical uncertainties across the full range of detector resolutions.
    %
    Error bars represent the standard error of the mean over the ensemble of pseudo-experiments. The ratio panels confirm parameter recovery within uncertainties, of truth values, validating the unfolding procedure's ability to correct for detector effects without introducing systematic biases.
    }
...
\end{figure}
            ...
            %
            \cref{fig:mu_mean_values_with_errorbars} and \cref{fig:var_mean_values_with_errorbars} summarise the accuracy or bias of the method, showing the mean fitted $\mu$ and $\sigma^2$ across 500 trials as a function of the detector smearing.
            %
            \cref{fig:uncertsfullybinned} focuses on the precision and uncertainty coverage, comparing the nominal $1\sigma$ errors from the fits to the actual spread of the results.
            %
            ...
            %
            For all tested smearing levels, the average fitted mean $\E\qty[\hat{\mu}]$ remains within statistical uncertainty of the true value, \(0.2\), and the average fitted variance $\E\qty[\hat{\sigma}^2]$ within statistical uncertainty of \(0.81\).
            %
            In \cref{fig:mu_mean_values_with_errorbars} and \cref{fig:var_mean_values_with_errorbars}, the averaged best fit values lie on or around the horizontal red dashed lines marking the true parameters.
            %
            ...
            %
            The pink square markers in \cref{fig:uncertsfullybinned} show the average fit uncertainty on $\mu$ and $\sigma^2$ obtained when using only the diagonal uncertainties.
            %
    ...
    \label{subsec:weight_correlations}
        ...
        \begin{enumerate}
            ...
            \item The \textbf{bin--bin covariance matrix}, \(\Sigma\) of a fine histogram binned {after} unfolding.
        \end{enumerate}
        
        Both are evaluated for four detector resolutions \numlist{0.00;0.25;0.50;0.75} and for two distinct estimators, KDE and neural network (NN) conditional density estimators.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption[]{(continued from previous page) The characteristic correlation length scales approximately linearly with detector resolution. Figure created by Owen Long~\cite{Desai:2025mpy}.}
\end{figure}
        ...
        For every event, by considering the weight distributions across pseudo--experiments one can compute the correlation coefficient, \(\rho\) as defined in \cref{def:correlation}.
        %
        One can then average $\rho_{ij}$ in bins of $\Delta x = |x_i-x_j|$, where $x$ is the unfolded observable.
            %
            ...
            \begin{description}
            \item[Perfect detector ($\sigma_{\det}=0$):] One should expect the average pairwise correlation to be $\rho(|\Delta x|) \simeq 0$ for all separations $\Delta x>0$, confirming that when no smearing is applied, \textsc{OmniFold} reproduces the {factorised likelihood} limit in which event weights are statistically independent~\cite{andreassen_omnifold_2020,Milton:2025mug}.
                %
            ...
            \item[Mild smearing (\(\sigma_{\det}=0.25\)):]
                %
                ...
                \[
                  ...
                  %
                  For both the NN and the KDE, \(\rho\) exhibits the same damped oscillatory structure, as would be expected by the normalisation imposed by the unfolding.
                  %
            ...
            \item[Moderate and large smearing ($\sigma_{\det}\ge0.5$):] Correlations develop a br\-oad plateau independent of method choice.
                %
                In that regime $N_\text{eff}$ collapses explaining why a na\"ive fit ignoring  correlations would significantly misestimate errors.
            \end{description}
        ...
        \subsubsection{Histogram covariance matrices.}
            Since most downstream analyses eventually bin the unfolded events, a histogram covariance matrix can be effective to visualise the correlations.
            %
            ...
            \[
              \Sigma_{ab} = \E\qty[(N_a-\bar N_a)(N_b-\bar N_b)].
            \]
            
            The matrices, shown in \cref{fig:weight-histogram-covariance-1d}, reinforce the observations from the correlation curves.
            %
            Even at zero smearing, while KDE produces a nearly diagonal $\Sigma,$ \textsc{OmniFold}'s weights exhibit small but noticable off diagonal elements.
            %
            ...
            Such strong off diagonal structure rigorously explains why a $\chi^2$ fit that ignores covariances by using only the diagonal of $\Sigma$ leads to significant miscoverage.
\begin{figure}
    \centering
    \begin{subfigure}[b]{\textwidth}
        \centering
        \includegraphics[width=0.95\textwidth]{figures/chapter-07/weight-histogram-covariance-1d.pdf}
        \caption{KDE-based \textsc{OmniFold}: Covariance matrices exhibit concentrated correlation structures with significant values ($>0.1$) confined primarily to nearest and next to nearest neighbour bins.}
        \label{fig:covar-kde}
    \end{subfigure}
    \stepcounter{footnote}
    ...
    %
     \textbf{(a)} displays results from KDE and \textbf{(b)} shows NN results, both using \textsc{OmniFold}.
     %
     Both methods exhibit a clear progression from nearly diagonal matrices at perfect resolution to increasingly correlated structures as detector resolution degrades.
    %
...
\begin{figure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \caption{NN-based \textsc{OmniFold}: Covariance matrices exhibit broad correlation bands extending $\ge10$ bins from the diagonal at $\sigma_{\text{det}} = 0.75$.}
        \label{fig:covar-nn}
    \end{subfigure}
    \caption[]{(Continued from previous page) 
    %
    These covariance matrices are essential inputs for rigorous statistical analysis of unfolded data, and must be propagated through any subsequent fits or hypothesis tests to obtain correct uncertainties. Figure produced by Owen Long~\cite{Desai:2025mpy}.}
\end{figure}
    ...
    \label{subsec:unbinned_data}
        ...
        %
        Equivalently, one finds the $\Delta\ln\mathcal L=0.5$ offset for a $1\sigma$ interval.
...
\begin{figure}
    ...
    \caption[Comparison between binned and unbinned parameter inference with unbinned and binned unfolding methods as a function of detector resolution.]{\small Comparison of inference uncertainty and bias for distribution parameters extracted from unfolded data as a function of $\sigma_{\text{det}}$.
    %
    ...
    %
    Three unfolding and inference approaches are compared: (i) Unbinned Unfolding (UBU) with unbinned maximum likelihood (ML) fitting shown as blue circles, (ii) binned Iterative Bayesian Unfolding (IBU) with $\chi^2$ fitting using the full covariance matrix (orange triangles), and (iii) IBU with $\chi^2$ fitting using only diagonal covariance elements (green squares). The blue shaded band in each panel represents $\pm$ RMS inference uncertainty from the UBU+ML approach. Figure created by Owen Long~\cite{Desai:2025mpy}.
    }
...
\end{figure}
            ...
            %
            This can be explained as both fits ultimately maximising a likelihood, or minimising a $\chi^2$, to match the unfolded distribution to the model;
            %
        ...
        \subsubsection{Uncertainty estimation and coverage.}
            ...
            %
            The uncertainty degradation with increasing $\sigma_{\text{det}}$ follows the expected $\displaystyle\sqrt{1 + \frac{\sigma^2_{\text{det}}}{\sigma^2_{\text{true}}}}$ scaling for both parameters.
        ...
        %
        In higher dimensions the unbinned approach is expected to drastically outperform a binned analysis (since binning in many dimensions is impractical or introduces large discretisation errors)~\cite{Pan:2024rfh, Butter2023MachineGeneration, Plehn:2022ftl}.
    ...
    \label{sec:highD_extension}
        ...
        %
        The Gaussian parameters used in the \numlist{2;4;6} dimensional studies are listed in \cref{tab:highD-params-2d,,tab:highD-params-4d,,tab:highD-params-6d} respectively.
        %
...
\begin{figure}
    ...
    \caption[Validation of asymptotic uncertainty estimates across dimensionalities in unbinned unfolding.]{\small Comparison of empirical RMS parameter uncertainties with mean asymptotic uncertainty estimates for all model parameters across \(d = \numlist{1;2;4;6}\). Each point represents a single parameter (mean or covariance matrix element) from the multivariate Gaussian.
    %
    The blue circles denote components of the mean, and the orange diamonds denote components of the covariance.
    %
    The grey dashed line indicates perfect agreement, while the green solid line shows the least squares fit for each dimension. 
    %
    The systematic deviation above the unit line demonstrates that asymptotic uncertainties consistently underestimate the true parameter uncertainties.
    %
    The increasing scatter with dimension indicates heterogeneous behaviour across different parameter types, with off diagonal correlation parameters typically showing larger deviations than mean parameters. Figure created by Owen Long~\cite{Desai:2025mpy}.
    }
...
\end{figure}
            ...
            \cref{fig:rms-asy-6D-res} repeats the \(d = 6\) study with detector resolution scaled by \numlist{0;1;2;3} relative to the $\sigma_{\det}$ in \cref{tab:highD-params-6d}.
            %
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figures/chapter-07/nn-uncertainties-6d-res-var.pdf}
    \caption[Impact of detector resolution on asymptotic uncertainty reliability in 6 dimensional unfolding.]{Comparison of empirical RMS uncertainties with asymptotic estimates for the six dimensional unfolding study for \numlist{1;2;3;4}$\,\times\, \boldsymbol{\sigma}_{\text{det}}$ listed in \cref{tab:highD-params-6d}.
    %
    The grey dashed line indicates perfect agreement, while green line shows least squares fits for each smearing level.
    %
    The RMS/asymptotic ratio increases monotonically with detector smearing, from 1.15 at perfect resolution to 1.45 at \(3\times\) smearing, demonstrating that asymptotic uncertainties become increasingly optimistic as detector effects worsen. This degradation arises from increased correlations induced by the unfolding process.
    %
    The vertical spread of points also increases with smearing, indicating that the reliability of asymptotic estimates becomes more parameter dependent. Correlation parameters (off-diagonal elements of $\boldsymbol{\rho}$) show particularly poor estimation at high smearing, with some parameters exhibiting RMS/asymptotic ratios exceeding 2.0. Careful uncertainty validation is essential when applying unbinned unfolding to experiments with significant detector effects. Figure created by Owen Long~\cite{Desai:2025mpy}.
    }
...
\section{Conclusions and outlook.}
    ...
    %
    This validates the intuitive principle that one should delay information reduction as much as possible in the analysis chain, thereby maximising the use of available information.
    %
