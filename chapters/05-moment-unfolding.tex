...
Varying $d$ in \eqref{eq:mlc-perfect} gives
\[
    \frac{\delta L}{\delta d(z)} = -\frac{p(z)}{d(z)+\qt(z)}.
\]

Hence $d^\star(z)=\nicefrac{p(z)}{\qt(z)}$. Substituting and using $\int p=\int \qt=1$ gives \eqref{eq:Lstar}.
\end{proof}
...
\end{gather}
At any stationary point, this vanishes for $a=1,\dots,n$, proving \eqref{eq:moment-match-perfect}.
\end{proof}

\subsubsection{Universal but imperfect detector response.}

For $g$ as in \eqref{eq:g}, define
...
\end{gather}
where $p_{\mathrm{mod}}$ is as in \eqref{eq:pmod}. Stationarity enforces \eqref{eq:moment-match-univ}.
\end{proof}
...
\end{gather}
If $r(x\mid z)=\delta(x-z)$, then $\tilde q(z\mid x)=\delta(z-x)$ and \eqref{eq:f} gives $f(z\mid z')=\delta(z-z')$.
\end{proof}
